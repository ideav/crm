...
Use the checked-in terminal script for reviewed cleanup. It is dry-run by
default and only considers remote branches whose latest GitHub pull request is
merged:
...
default it only considers `issue-*`, `fix-*`, `fix/*`, `feature/*`, and
`revert-*` branches. Use `--help` to see all options.
