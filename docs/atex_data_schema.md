...
| 1081 | Партия ГП | DATETIME (дата прихода) | Партия готовой продукции (⊂ «Задание в производство») |
...
| 82374 | Номер джамбо | SHORT (номер джамбо) | Расход конкретного джамбо под заданием (⊂ «Задание в производство», см. §6.7) |
...
erDiagram
    ...
    "Вид сырья"            ||--o{ "Заказанное количество"        : "вид"
    "Слиттер"             }o--o{ "Вид сырья"             : "стоп-лист (multi)"
    "Клиент"               ||--o{ "Заказ"                 : "заказчик"
    "Пользователь"         ||--o{ "Заказ"                 : "менеджер"
    "Заказ"                ||--o{ "Заказанное количество"        : "содержит ⊂"
    "Заказанное количество"       ||--o{ "Обеспечение"           : "покрывается ⊂"
    "Заказанное количество"       ||--o{ "К-во план"             : "задание на втулки ⊂"
    "Диаметр втулки"       ||--o{ "Заказанное количество"        : "втулка"
    ...
    "Производственная резка" ||--o{ "Номер джамбо"         : "расход джамбо ⊂"
...
| Отпуск | DATETIME → подчинённая **Отпуск** (122572) | #3764: окна простоя станка (ТО и т.п.), `arr_id` 122572 |
...
**Отпуск (122572)** — подчинённая таблице «Слиттер» (создаётся с `up`=id станка). Диапазон
дат-времён, когда станок не принимает задания; ведётся для планирования ТО и подобных событий
(#3764). Реквизиты:

| Реквизит | Тип | Примечание |
|----------|-----|------------|
| **(имя)** | DATETIME (unix-сек) | Начало окна простоя — главное значение записи |
| Окончание | DATETIME (unix-сек) | Конец окна простоя |
| Примечания | MEMO | Причина (ТО и т.п.) |

Автогенерация очереди (`production-planning.js`) пропускает эти окна: резки, попадающие на простой,
выталкиваются за его конец (`shiftPlacementsPastDowntime` поверх `buildSchedule`/`splitMachineQueue`).
Ручное добавление задания на любой период не ограничивается. Кнопка «Отпуск {станок}» (правее
«Удалить») открывает редактор окон активного станка.
...
### 5.3 Задача на втулки (1080) ⊂ Заказанное количество — «Задание на втулки»

Задание для пульта втулкореза; подчинено позиции заказа. Таблица резолвится по имени и по
`alias` «Задание на втулки» (см. [#3159](https://github.com/ideav/crm/issues/3159),
[#3140](https://github.com/ideav/crm/issues/3140)). **Главное значение (первая колонка) —
ДАТА/ВРЕМЯ планового старта** (Unix), не количество. Отдельного поля «Статус» нет — статус
выводится из заполненности «Начато»/«Закончено»/«Кол-во факт».

| Реквизит | Тип | Примечание |
|----------|-----|------------|
| **(главное значение)** | DATETIME | Плановый старт задания (Unix) |
| Втулкорез | SHORT → **Втулкорез** | Назначенный станок |
| Кол-во | NUMBER | Плановое количество втулок |
| Кол-во факт | NUMBER | Фактически нарезано |
| Начато | DATETIME | Старт выполнения |
| Закончено | DATETIME | Завершение (заполнено → задание терминально) |
| Партия сырья | SHORT (multi) → **Партия сырья** | FIFO-партия |

Пульт втулкореза читает задания отчётом `sleeve_tasks` (подчинённую таблицу нельзя прочитать
плоско) с серверным фильтром по втулкорезу и дню; см. [§3.6](atex_workplaces.md). Статус:
`Закончено`+факт>0 → Готово; `Закончено`+факт=0 → Пропущена; `Начато` → В работе; иначе Ожидает.
...
| Кол-во резок план | NUMBER | Проходов по плану |
| Кол-во резок факт | NUMBER | СДЕЛАНО проходов (657315). Единственный источник этого числа: его пишет пульт слиттера при каждой отметке ✓ Готово, читают заголовок «Резка N из M» и «Урегулировать» (разделение частично выполненного задания, #4564). Пусто = не начинали |
| Начато | DATETIME | Момент старта работы на станке |
| Закончено | DATETIME | Момент завершения. «Урегулировать» ставит его выполненной части разделённого задания — концом смены её фактического дня (#4564) |
| Счётчик нач. | NUMBER | Остаток джамбо на старте (из партии сырья); счётчик джамбо убывает |
| Счётчик кон. | NUMBER | Остаток на финише = нач. − погонаж (ввод наоборот даёт 0, #4321); дублируется в запись «Номер джамбо» (§6.7) |
| Погонаж факт, м | SIGNED | Фактический погонаж = «Счётчик нач.» − «Счётчик кон.». Накапливается отметками пульта: прежнее значение + метраж отмеченных резок + рабочий расход + списание + брак (#4902). Числом проходов НЕ является: у не начатой резки он равен остатку партии (#4351) |
| Брак, м² | SIGNED | Не заполняется пультом (#4914) |
| Примечания | MEMO | |
| Брак, м | SIGNED | Не заполняется пультом (#4914): брак ведётся в записи «Номер джамбо» (§6.7) |
| Фото брака | FILE | Не заполняется пультом (#4914): фото ведётся в записи «Номер джамбо» (§6.7) |
...
Расход джамбо у резки вели реквизитами «Рабочий расход, м» (787042), «К списанию, м»
(787043), «№ джамбо» (787045) и «Брак, шт» (785730) — выведены из эксплуатации (#4914):
пульт их не пишет и не читает, расход ведётся записями «Номер джамбо» (§6.7), значения
перенесены туда и реквизиты очищены.
...
Партия готовой продукции, выпущенная из задания. **Подчинённая таблица «Задания в
производство»** (master-detail, `up` = задание): выпуск задания держится строками «Партия ГП»
под ним; отдельного реквизита-ссылки на задание у партии нет — связь выражена подчинением.
Главное значение — «Дата прихода» (DATETIME), которое сервер проставляет сам и которое служит
ключом FIFO при отгрузке.

| Реквизит | Тип | Примечание |
|----------|-----|------------|
| **(дата прихода)** | DATETIME | Главное значение, ключ FIFO |
| Ширина, мм | SIGNED | Ширина рулонов партии |
| Кол-во полос | NUMBER | Полос (ножей) в партии |
| Кол-во рулонов | NUMBER | |
| Метраж, м | SIGNED | |
| Адрес хранения | SHORT | Место на складе ГП |
| В работе | BOOL | Признак активности партии |
| Штрих-код | SHORT | |
| Кол-во план | NUMBER | План выпуска |
| Кол-во факт | NUMBER | Факт выпуска |
| ID заказа | SHORT | #3433: привязка к заказу |
...
**Количественный лимит (#3445).** Главное значение (первая колонка) — не только признак
«стокабельности», но и максимально допустимый запас в рулонах. На склад по номенклатуре нельзя
нарезать больше этого значения **с учётом текущего остатка**: при планировании читается
остаток ГП (`loadStockBalance` — «Партии ГП» не в статусе «Отгружен» по той же номенклатуре),
свободный лимит = `maxStockLimit − остаток` (`stockHeadroom`). Авто-генерация урезает добор и
перепроизводство впрок до этого лимита (`capStockToHeadroom`); ручной редактор полос блокирует
ввод/добор сверх лимита. Заказное покрытие не ограничивается.

### 6.7 Номер джамбо (82374)

Расход конкретного джамбо под заданием. Джамбо хранится одной партией (16–20 км), и помимо
нарезанного метража из него тратится протяжка перед резкой («Рабочий расход») и остаток под
утиль («К списанию») — эта таблица держит историю такого расхода по каждому заданию.
**Подчинённая таблица «Задания в производство»**: записи создаются с `up` = id задания; их
несколько — по одному на каждый джамбо, из которого резали (пульт переключает их корешками-
закладками над панелью показаний, #4914/#4916). Главное значение — сам номер джамбо (SHORT):
его видит упаковщик (плашка в РМ
упаковки, отчёт `task_jumbo` — integram-reports.md §15).

| Реквизит | id | Тип | Примечание |
|----------|----|-----|------------|
| **(номер джамбо)** | 82374 | SHORT | Главное значение |
| Начальная длина, м | 82376 | SHORT | «Счётчик нач.» задания на старте этого джамбо |
| Счётчик нач. | 791706 | SHORT | Остаток джамбо на старте; у следующего джамбо задания — «Счётчик кон.» предыдущего |
| Кол-во резок | 82378 | SHORT | Сколько резок сделано из ЭТОГО джамбо: копится отметками проходов в активную запись (#5005); при завершении сумма по записям сверяется с «Кол-вом резок факт» задания — недостача доносится в активную запись |
| Счётчик кон. | 791707 | SHORT | Остаток после резок ЭТОГО джамбо: отметки проходов ведут его сами (погонаж записи = «Счётчик нач.» − «Счётчик кон.», #5010); завершение только доводит пустое значение по резкам записи (записи до #5010) |
| Конечная длина, м | 82380 | SHORT | «Счётчик кон.» за вычетом рабочего расхода и списания |
| Рабочий расход, м | 82382 | SHORT | Протяжка перед резкой; копится отметками проходов |
| К списанию, м | 82384 | SHORT | Остаток джамбо, идущий в утиль; копится отметками |
| Брак, м | 82386 | SHORT | Брак по погонажу; копится отметками |
| Брак, шт | 791708 | SHORT | Брак по количеству; копится отметками |
| Фото брака | 791712 | FILE | Снимок брака |

Баланс джамбо: `Конечная длина = Счётчик кон. − Рабочий расход − К списанию`, а потребление
партии сырья (разница счётчиков = «Погонаж факт» = «Расход сырья») включает рабочий расход
и списание — партия списывается на всё ушедшее джамбо. Пишет пульт слиттера
(`download/atex/js/slitter.js`, #4914): ввод в показаниях сохраняется автосохранением при
выходе из ячеек в запись АКТИВНОГО джамбо, отметка прохода прибавляет введённое к накопленным
значениям записи (поля сразу показывают накопленное), при завершении резки записываются длины
и счётчик. На задании без записей номер джамбо НЕ подставляется — оператор вводит его
руками (#4933 п.2). Расход без номера джамбо не сохраняется. #5005: отметка прохода прибавляет свои
резки к «Кол-ву резок» активной записи — по записям джамбо видно, сколько резок с какого
джамбо (упаковщик считает по ним «N шт с джамбо X» с учётом «Брака, шт»); завершение резки
сверяет сумму резок по записям с фактом задания и доносит недостачу в активную запись.
Реквизиты в схеме типа SHORT — метры хранятся
строкой.

Расходные реквизиты самой резки — «Рабочий расход, м» (787042), «К списанию, м» (787043),
«№ джамбо» (787045), «Брак, м» (8458), «Брак, шт» (785730), «Фото брака» (8460), «Брак, м²» (82) —
выведены из эксплуатации (#4914): пульт их не пишет и не читает, значения перенесены в записи
«Номер джамбо» и в живой базе очищены.
...
4. **Оператор слиттера** ведёт резку: меняет «Статус», вводит счётчики (нач./кон.) → «Погонаж
   ...
   смены». Расход конкретного джамбо (номер, рабочий расход, к списанию) пишется в
   `Номер джамбо` ⊂ резки.
...
| Производственная резка | Партия ГП ⊂ | Партия ГП | 1:N |
...
- **«Партия ГП» (1081) подчинена «Заданию в производство»** (master-detail, `Партия ГП` ⊂
  `Задание`): выпуск задания хранится строками «Партия ГП» под ним, отдельного реквизита-ссылки
  на задание у партии нет. Поэтому удаление задания каскадом (`BatchDelete`) сносит его «Партии
  ГП»/«Полосы»/«События смены» — но сначала надо удалить «Обеспечения», которые ссылаются и на
  задание, и на его «Партии ГП» (иначе `_m_del` задания → 409 «есть ссылки»). Этот порядок
  (обеспечения → задание) и реализуют `runDeleteCutTask`/`runDeleteDayTasks`.
