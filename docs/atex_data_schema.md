...
| Погонаж факт, м | SIGNED | Фактический погонаж (= кон. − нач.). Числом проходов НЕ является: у не начатой резки он равен остатку партии (#4351) |
...
**Подчинённая таблица «Задания в производство»**: связь — реквизит «Номер джамбо» (82389,
`orig=82374`) у «Производственной резки» (1078), записи создаются с `up` = id задания.
Главное значение — сам номер джамбо (SHORT): его видит упаковщик.

| Реквизит | id | Тип | Примечание |
|----------|----|-----|------------|
| **(номер джамбо)** | 82374 | SHORT | Главное значение |
| Начальная длина, м | 82376 | SHORT | «Счётчик нач.» задания |
...
| Рабочий расход, м | 82382 | SHORT | Протяжка перед резкой |
| К списанию, м | 82384 | SHORT | Остаток джамбо, идущий в утиль |
| Брак, м | 82386 | SHORT | Дублируется из задания для истории джамбо |
| Фото | 82388 | SHORT | Не заполняется пультом слиттера |
...
(`download/atex/js/slitter.js`, #4860): автосохранением при выходе из ячеек «Номер джамбо» /
«Рабочий расход, м» / «К списанию, м» и финально при завершении резки; расход без номера
джамбо не сохраняется. Реквизиты в схеме типа SHORT — метры хранятся строкой.
