...
- **Два потока дохода партнёра:** (1) разовая доля с проектов; (2) рекуррент с поддержки, доработок и абонплаты. Масштаб — обученные адепты ведут больше заказчиков параллельно, а партнёр только наращивает канал; тиражное готовое решение продаётся многим (с отчислением **60–80%** платформе). Два направления (слайд 4) = два потока сделок: сателлиты в корпорациях + крупные ключевые системы в МСБ.
