...
- **Что даёт:**
  ...
  - **Сублинейный поиск, recall не проседает с ростом N** — воспроизводимый proof-тест в пакете (метрика **recall@10**): держится плоско **0.83@400 → 0.82@900**, тогда как плоский NSW при том же бюджете оседает до **0.75@1000**; число косинусов на запрос растёт сублинейно (~0.29N при N=900), обход детерминирован.
  - **На большом масштабе** — отдельный замер на живой Integram (метрика **recall@1**, не @10): **×12** быстрее точного brute при N=5 000 (95%), **×86** при N=50 000 (92%).
...
- **Пруф:** recall@10 — воспроизводимый proof-тест `packages/vecmory/test/hnsw-proof.test.ts` (github.com/ideav/python2node, #83); recall@1 и скорость на масштабе — vecmory/summary_vm.md (github.com/ideav/crm, замер на живой Integram, сравнение с pgvector/Neo4j).
...
- **Команда:** CEO Алексей Семёнов (разработчик ядра; Citi, MTS, Neoflex, ВТБ, Сбер), CTO Александр Орехов (highload, 15 лет), аналитики Артамонов Ким и Денис Гаврилов, разработчик Дмитрий Гречаный; **+300 адептов**.
