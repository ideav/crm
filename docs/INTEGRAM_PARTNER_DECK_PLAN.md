...
- **Кто есть кто в схеме (роль → что делает → на чём зарабатывает):**
  ...
  | **Партнёр (вы)** | инициирует и одобряет сделки, приводит своих заказчиков и бренд; **внедрением и поддержкой не занимается** | **доля с проектов и подписок — без операционной нагрузки и без работы руками** |
  | **Адепт** | делает всю работу: собирает решение на Интеграме, внедряет, ведёт поддержку и **отвечает за результат** перед заказчиком (в штате партнёра или на подряде) | оплата работы — внутри чека проекта |
...
- **Что значит «партнёр зарабатывает на проектах заказчика»:**
  - Партнёр **инициирует и одобряет** сделку и отдаёт её адепту — тот собирает решение на Интеграме (чек 37,5 тыс – 500 тыс ₽, слайд 14) и ведёт поддержку. Платформе идёт только плата за инструмент (подписка **от 1 950 ₽/мес** или лицензия **290 тыс ₽/год**) — это стоимость платформы, **а не комиссия с проекта**. Партнёр получает **свою долю, не занимаясь реализацией**.
...
- **Два потока дохода партнёра:** (1) разовая доля с проектов; (2) рекуррент с поддержки, доработок и абонплаты. Масштаб — обученные адепты ведут больше заказчиков параллельно, а партнёр только наращивает канал; тиражное готовое решение продаётся многим (с отчислением **20–50%** платформе). Два направления (слайд 4) = два потока сделок: частые сателлиты в корпорациях + крупные ключевые системы в МСБ.
