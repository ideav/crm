...
- **Плашки-регалии:** Реестр российского ПО Минцифры №30872 · Патенты РФ + US · Победитель SberUp · Продукт недели #1 Product Radar.
...
- **Заголовок:** В основе — квинтетная модель данных (QDM). Не реляционка, не EAV.
- **Контент:**
  ...
  - Одно ядро покрывает функциональность РСУБД + NoSQL + колоночных БД; масштабируется распределением по префиксам ID.
  - **Защита:** патенты **RU 2650032 C1** и **US 11138174 B2** (одна семья, приоритет 2017, US действует до 2038).
...
- **Пруф/источник:** статья Neoflex про квинтет — habr.com/ru/companies/neoflex/articles/433058/
...
> больше и это НИКОГДА не решается конструктором».*
...
- **Как решает Интеграм (проверенные цифры):**
  - **32 млрд записей / 4,5+ ТБ** в одной модели, 703+ млн бизнес-транзакций.
  - Точный поиск по 700 млн записей — **0,77 с**; фильтр по одному из 45 полей — **1,28 с**.
...
- **Плашка-удар:** «Это лишь несколько примеров — их гораздо больше. Традиционное решение = кратно больше ресурсов и сложности; конструктором это НЕ решается никогда. Значит, вы берёте проекты, от которых отказываются другие».
...
- **Направление «замена Excel» (сателлиты, короткий цикл):**
  - Тип 1 — учёт договоров, платежей, рабочего времени → **чек 37 500 ₽**.
  - BI/аналитика/планирование поверх выгрузок из основной системы.
- **Направление «ключевая система» (МСБ core, крупный чек):**
  - Тип 2 — контроль и обслуживание аппаратов (вендинг воды) — операционное ядро сервисной компании → **чек 150 000 ₽**.
  - Тип 3 — рекрутинговый стартап, поточный найм 200–500 чел. — ключевая система бизнеса → **чек 500 000 ₽**.
...
- **Визуал:** 3 скриншота-кейса, сгруппированы по двум направлениям + крупные плашки чеков.
...
- **Заголовок:** Партнёрская модель — как у 1С, Тильды, AmoCRM, Bubble. Партнёр берёт продукт в портфель и зарабатывает на проектах своих заказчиков.
- **Кто есть кто в схеме (роль → что делает → на чём зарабатывает):**
  ...
  | **Партнёр (вы)** | берёт продукт в портфель, продаёт своим заказчикам, организует внедрение и поддержку | **маржа проекта + поддержка/абонплата + перепродажа подписок/лицензий** |
  | **Адепт** | эксперт-внедренец: собирает решение на платформе (в штате партнёра или на подряде) | оплата работы — внутри чека партнёра |
  | **Заказчик** | конечный клиент партнёра | платит: партнёру — за проект и поддержку, платформе — за подписку |

- **Что значит «партнёр зарабатывает на проектах заказчика»:**
  - Партнёр продаёт заказчику решение (чек 37,5 тыс – 500 тыс ₽, слайд 14) и собирает его на Интеграме силами адепта. Платформе идёт только плата за инструмент (подписка **от 1 950 ₽/мес** или лицензия **290 тыс ₽/год**) — это стоимость платформы, **а не комиссия с проекта**. Всё сверх стоимости инструмента и работы адепта — **маржа партнёра**.
  - **Пример:** ключевая система для МСБ, чек 500 тыс ₽. Заказчик платит партнёру за проект, а дальше — за поддержку и доработки; платформе — подписку/лицензию за использование. Один заказчик = разовый проект + длинный хвост рекуррентной поддержки.
- **Два потока дохода партнёра:** (1) разовая проектная выручка с внедрений; (2) рекуррент с поддержки, доработок и абонплаты. Масштаб — обученные адепты ведут больше заказчиков параллельно; тиражное готовое решение продаётся многим (с отчислением **20–50%** платформе). Два направления (слайд 4) = два потока сделок: частые сателлиты в корпорациях + крупные ключевые системы в МСБ.
- **Схема-стрелка (из слайда 10 «Распак»):** Платформа обучает адептов → Партнёр (силами адептов) внедряет и поддерживает проекты → Заказчик платит партнёру за проект и поддержку, а платформе — за подписку.
...
- **Цель:** показать, что это работающий бизнес с растущей выручкой и большим рынком, а не идея.
- **Заголовок:** Растём кратно на собственных ресурсах. Рынок — на десятки процентов в год.
- **Трекшн (слайд 12 «Распак»):** выручка 2022 → 2023 → 2024 = **0,2 → 0,7 → 3,1 млн ₽** (×4–5 в год); 70 активных клиентов РФ/СНГ; 5 лицензий продано; 5 партнёров + 7 адептов зарабатывают; +300 адептов; финалист/победитель SberUp; Продукт недели #1 Product Radar; реестр Минцифры №30872.
...
- **Юнит-экономика для партнёра:** подписка (рекуррент) + чек проекта 37,5 тыс – 500 тыс ₽ + доля с готовых решений; окупаемость обучения адепта — на первом-втором проекте.
- **Визуал:** график выручки по месяцам + TAM/SAM/SOM-круги.

### Слайд 18 — Команда, пруфы, оффер партнёру
- **Заголовок:** Опытная команда, проверяемая технология, готовы дать доступ.
- **Команда (слайд 11 «Распак»):** CEO Алексей Семёнов (разработчик ядра; Citi, MTS, Neoflex, ВТБ, Сбер), COO Юлия Голубкова (12 лет управления проектами), CTO Александр Орехов (highload, 15 лет), аналитики Артамонов Ким и Денис Гаврилов; **+300 адептов**.
...
- **Оффер партнёру / next steps:** доступ к демо и репозиториям · пилотный проект · условия партнёрства и обучение адептов · контакты (integram.io, tg: @qdmadept).
...
7. **Слайд монетизации (16) переписан под вопрос «где здесь партнёр»:** карта ролей (платформа / партнёр / адепт / заказчик — кто что делает и на чём зарабатывает) и расшифровка «партнёр зарабатывает на проектах заказчика» на цифрах (маржа проекта + рекуррент с поддержки; плата платформе — за инструмент, а не комиссия с проекта).
