...
## 2. Предусловие: пользователь с токеном и роль в Integram

Токен сам по себе — только *аутентификация*; прав на данные у него по умолчанию нет
(запрос вернёт `200` с телом `"...нет доступа... Ваш глобальный доступ: ''"`).

Доступ в Integram складывается из трёх вещей: **пользователь** (под ним ходит Airflow),
его **роль** (у пользователя ровно одна роль) и **гранты роли на объекты** — какие таблицы
и отчёты роль читает и на каком уровне (READ / WRITE / BARRED). Токен — это строка в поле
«Токен» пользователя; по нему клиент ходит в обход формы входа и капчи (логин/пароль
машинному клиенту не подходят).

Пользователь, роль и гранты — обычные **системные таблицы** Integram и правятся как любые
данные. Что и где создать:

### 2.1. Создать роль
Открыть системную таблицу **«Роль»** (тип `42`) → добавить запись:
- **Название** — напр. `DWH / Airflow (read-only)`;
- **Описание** — назначение роли.

### 2.2. Выдать роли грант на чтение отчёта
У роли есть подчинённая таблица грантов **«Объекты»** (тип `116`). Добавить в неё запись:
- **Объект** — что открываем:
  - `17021` — только этот отчёт (**рекомендуется**, least-privilege для DWH);
  - `22` — таблица «Запрос», т.е. все отчёты (если токен тянет много выгрузок).
- **Уровень** — `READ`.

По умолчанию новый отчёт не выдан ни одной роли — без этого гранта API отвечает ошибкой доступа.

### 2.3. Создать пользователя с токеном
Открыть системную таблицу **«Пользователь»** (тип `18`) → добавить запись:
- **Логин** — напр. `airflow` (или `dwh`);
- **Роль** — созданная в 2.1;
- **ФИО** — напр. `DWH Airflow (сервис)`;
- **Токен** — строка **15–30 символов**; **это и есть токен для Airflow** (§3).
  Логин/пароль машинному клиенту не нужны.

Служебные поля (`Activity`, `xsrf`, `Retries`) заполняет сервер — их не трогать.

### 2.4. Проверить
Скопировать значение поля «Токен» и выполнить curl из §4 — отчёт должен отдать данные,
а не `{"error": …}`.

### Вариант через API (если удобнее скриптом)
Поля и уровни — из базы знаний Integram (`docs/kb/roles.md`). `_xsrf` берётся из
`GET /r7ohr/xsrf?JSON`; выполняется под токеном админа базы. Уровни: `52`=BARRED, `53`=READ, `54`=WRITE.

```bash
# 1) роль (t42=название, t135=описание)
POST /r7ohr/_m_new/42?JSON=1   -F token=<admin> -F _xsrf=<xsrf> -F up=1 \
     -F t42='DWH / Airflow' -F t135='read-only'
# 2) грант роли на чтение отчёта 17021  (t116=объект, t136=53=READ)
POST /r7ohr/_m_new/116?JSON=1  -F token=<admin> -F _xsrf=<xsrf> -F up=<roleId> \
     -F t116=17021 -F t136=53
# 3) пользователь с токеном (t18=логин, t115=роль, t125=токен, t33=ФИО)
POST /r7ohr/_m_new/18?JSON=1   -F token=<admin> -F _xsrf=<xsrf> -F up=1 \
     -F t18=airflow -F t115=<roleId> -F t125=<ТОКЕН> -F t33='DWH Airflow'
```

> Транспорт токена зависит от инстанса. Для чтения отчёта на `integram.io` проверен
> заголовок `Authorization: Bearer` (§3). Для write-операций `_m_*` действует правило
> Integram: form-поле `token=` + `_xsrf` в теле POST (заголовок `Authorization`/`?token=`
> в query для записи не годятся). Перед скриптовым созданием сверьте транспорт на своём инстансе.

### Least-privilege
Роль выдана правильно, если отчёт 17021 отдаёт данные, а корневой объект остаётся закрытым —
глобальный доступ на «всё» pull-токену не нужен и его выдавать не следует.
