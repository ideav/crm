...
- **Симптом:** «Копировать в буфер» подчинённой таблицы кладёт `1782968400` вместо `02.07.2026 08:00:00` → **Причина:** API отдаёт DATE/DATETIME unix-штампом; ячейку форматирует `formatSubordinateCellValue`, а копирование шло мимо него — `String(record.r[i])` (#4120) → **Фикс:** формат колонки (`normalizeFormat(req.type)`, для главного значения `metadata.type`) → `formatIntegramDateCellPlain` *(24-global-functions)*. Значение из `r[]` без типа колонки — сырое: штамп, `YYYYMMDD` или `"id:label"`.
- **Симптом:** подчинённая таблица standalone-формы (`window.openCreateRecordForm`/`openEditRecordForm`) показывает штамп/`"5:ТО"`/`1` вместо даты/метки/флажка → **Причина:** `renderSubordinateTableStandalone` выводила `escapeHtml(r[i])` без форматирования по типу (#4123) → **Фикс:** `formatSubordinateCellDisplay` *(25-create-form-helper)*, зеркало `formatSubordinateCellValue` *(19-form-edit)*.
- **Симптом:** в подчинённой таблице значения строки съехали на колонку влево, последняя пуста → **Причина:** `record.r[]` несёт слот на **каждый** реквизит, включая вложенные (`arr_id`, там счётчик записей); обход двигал индекс только по отрисованным колонкам (#4124) → **Фикс:** индекс продвигать на каждом реквизите, пропускать только отрисовку `arr_id`.
