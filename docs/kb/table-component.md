# Компонент таблиц `integram-table`
> Часть базы знаний Integram. Индекс: [docs/kb/README.md](README.md)

## На пальцах (для человека)

`integram-table` — готовый платформенный **data-grid** для рабочих мест: таблица с
бесконечным скроллом, фильтрами, сортировкой, группировкой, инлайн-правкой, формами
создания/редактирования записей, экспортом, вставкой данных из буфера и тёмной/светлой темой.
Собирается из 26 модулей `js/integram-table/*.js` в бандл `js/integram-table.js`.

**Когда брать:** почти любой список/таблица записей таблицы ИЛИ строк отчёта в рабочем месте —
вместо рукодельной вёрстки. Подключается **декларативно** одним `<div data-integram-table>`:
все фичи (фильтры/сорт/правка/экспорт/буфер) включаются сами. Это переиспользуемый кирпич —
по умолчанию стройте списки на нём.

> ────────── дальше — справочник для агента ──────────

## Операции (для агента)

### Встраивание (минимум) — пример из `templates/table.html`
```html
<div
    data-integram-table
    data-api-url="/{_global_.z}/metadata/{_global_.id}"
    data-page-size="20"
    data-cookie-prefix="tasks-object"
    data-title=""
    data-instance-name="tasksTable"></div>

<link rel="stylesheet" href="/css/integram-table.css" />
<script src="/js/integram-table.js"></script>
```
Компонент **авто-инициализируется** по элементам `[data-integram-table]` (отдельный `new
IntegramTable(...)` писать не нужно). Хэндл доступен глобально как `window.{instanceName}`
(напр. `window.tasksTable`) — для вызовов вроде `openPasteDataDialog()`.

### Источник данных — таблица ИЛИ отчёт (`data-api-url`)
Тип определяется по URL (`18-data-source.js getDataSourceType`):
- `/{db}/metadata/{id|имя}` или `/{db}/object/{id|имя}` → **таблица** (object/JSON_OBJ);
- `/{db}/report/{id|имя}` → **отчёт** (защищённый слой; см. [queries.md](queries.md)).
Имя вместо id резолвится через globalMetadata (#2302).

### Ключевые опции (атрибуты `data-*` → `this.options`, `01-core.js`)
| Атрибут | Опция | Назначение |
|---|---|---|
| `data-api-url` | apiUrl | источник (см. выше) |
| `data-page-size` | pageSize (20) | размер страницы (бесконечный скролл) |
| `data-cookie-prefix` | cookiePrefix | префикс для сохранения состояния (фильтры/колонки/сорт в cookie) |
| `data-instance-name` | instanceName | имя глобального хэндла `window.{name}` |
| `data-title` | title | заголовок над таблицей |
| `data-source-type`/`dataSource` | dataSource | 'report'/'table' (фолбэк, если URL неоднозначен) |
| — | tableTypeId | id типа для dataSource='table' |
| URL `?F_U=`/`?up=`/`?parentId=` | parentId | фильтр по родителю (подчинённые) |
| URL `?F_I=` | recordId | фильтр по id записи (#563) |

### Что доступно «из коробки» (с issue-ссылками)
- Фильтры (13+ операторов), сортировка, группировка, drag&drop порядка колонок, скрытие колонок, сохранение состояния в cookie.
- Инлайн-правка ячеек + формы создания/редактирования записей (права: `tableGranted` WRITE vs read-only, #1508).
- **Вставка данных из буфера** — кнопка `.paste-data-btn` → `window.{instanceName}.openPasteDataDialog()` (issue **#1606**); массовая вставка строк в таблицу.
- **Экспорт** (модуль `23-bulk-export`); **удаление по фильтру** при `metadata.delete==="1"` (#2749).
- **Копировать ID записи** (#563); **shareable URL-конфиг** — копировать ссылку на текущую конфигурацию таблицы (#510).
- Счётчик всего записей по клику «?» (#2795); тёмная/светлая тема (`data-theme`).

### Расширение / правка компонента
Бандл `js/integram-table.js` **генерируется** — НЕ редактировать напрямую. Править модули
`js/integram-table/*.js` (26 шт.) → `bash build.sh` из корня → коммитить и модуль, и бандл
(см. `CLAUDE.md`).

## Грабли
- **Симптом:** правки в `js/integram-table.js` пропадают/конфликтуют → **Причина:** это сгенерированный бандл → **Фикс:** править `js/integram-table/*.js` + `bash build.sh`, коммитить оба *(CLAUDE.md)*.
- **Симптом:** таблица не фильтруется по родителю/записи → **Причина:** не передан `F_U`/`up`/`parentId` (подчинённые) или `F_I` (#563) → **Фикс:** прокинуть через URL рабочего места *(01-core)*.
- **Симптом:** правка/создание недоступны, хотя ожидались → **Причина:** `tableGranted` ≠ WRITE (роль без права записи, #1508) → **Фикс:** выдать роли WRITE-грант на таблицу (см. [roles.md](roles.md)).
- **Симптом:** у супер-пользователя (uid='0') в таблице нет кнопок создания/правки записей и элементов структуры → **Причина:** сервер не строит ему карту грантов (Validate_Token пропускает getGrants), в метаданных ключа granted нет, а «granted нет» компонент трактует как READ (#4851) → **Фикс:** `isTableWritable()`/`isStructureWritable()` и нормализация granted в форме редактирования дают uid='0' полный доступ (#4892) *(01-core, 19-form-edit)*.
- **Симптом:** пересборка бандла (`bash build.sh`) уронила чужой тест (пример: `integram-table-4385.test.js`) → **Причина:** часть патчей живёт только в собранном бандле и не перенесена в модули (источник дрифта — коммит 85625f28 правил только бандл) → **Фикс:** перед пересборкой сверить `diff <(cat js/integram-table/*.js) js/integram-table.js`; расхождения перенести в модули, потом собирать *(PR #4893)*.
- **Симптом:** страница с несколькими `data-integram-table` шлёт одинаковый `/{db}/metadata` по разу на таблицу (N таблиц → N одинаковых ответов ≈15 кБ, #4252) → **Причина:** `globalMetadata` кешировался на экземпляре, каждый экземпляр качал схему сам → **Фикс:** `globalMetadata` кешируется **на весь класс** по `apiBase` (`IntegramTable._sharedGlobalMetadata`) — одна загрузка на страницу; после правки колонок кеш чистится `clearSharedGlobalMetadata()` (свежая схема), см. #2526 *(01-core `loadGlobalMetadata`/`clearSharedGlobalMetadata`)*.
- **Симптом:** перетащил колонку, нажал «Обновить» — значения уехали на соседнюю колонку, даты рисуются ссылками (#4364) → **Причина:** перетаскивание физически переставляет реквизит на сервере (`POST _d_ord/{id}?order=N`), после чего `object/{id}/?JSON_OBJ` раскладывает `r[]` по-новому; `this.columns` перечитывался только при пустых колонках или при расхождении **числа** колонок (#2526), а перестановка число не меняет — ячейка читается как `row[this.columns.indexOf(col)]`, то есть у соседа → **Фикс:** `saveColumnOrderToServer()` взводит `metadataStale`, и ближайшая полная перезагрузка (обновление/фильтр/сортировка) перечитывает метаданные таблицы; `refreshData()` взводит флаг всегда — «Обновить» подхватывает и чужие правки схемы *(01-core `shouldRebuildColumns`/`reloadTableMetadata`)*.
- Исторические грабли вёрстки/скролла компонента — в закрытых issues `ideav/crm` по `js/integram-table.js` (напр. дёрганье при чекбоксах/скролле): искать `is:issue is:closed integram-table`.

## Чек-лист: поставить рабочее место на `integram-table`

Для **нового** места-списка (или ретрофита существующего рукодельного):

1. **Источник.** Выбери: таблица (`/{db}/metadata/{id|имя}` или `/object/{id|имя}`) или отчёт (`/{db}/report/{имя}`). Для списков со связями/правами/join — предпочитай **отчёт** (защищённый слой, серверный SQL; см. [queries.md](queries.md)).
2. **Контейнер.** В шаблон РМ вставь `<div data-integram-table data-api-url=… data-page-size data-cookie-prefix data-instance-name>` + `<link …/css/integram-table.css>` + `<script …/js/integram-table.js>`. Инициализация — авто.
3. **Фильтр контекста.** Подчинённые/срез — прокинь `F_U`/`up`/`parentId` (родитель) или `F_I` (запись, #563) через URL/конфиг.
4. **Права.** Для инлайн-правки/форм выдай роли **WRITE-грант** на таблицу (иначе read-only, #1508; см. [roles.md](roles.md)).
5. **Кастом-логика — рядом, не внутри.** Спец-кнопки/формы (например «Согласовать», «Сгенерировать резки», нестандартные действия) оставь вне грида и вешай на глобальный хэндл `window.{instanceName}` или хуки `onCellClick`/`onDataLoad`. Грид не переписывай под бизнес-логику.
6. **Убери рукодельное.** Свой рендер таблицы, пагинацию, фильтры, сортировку — удалить: их даёт компонент. Колонки `*_id`/`*_Стиль`/`*_style` он скрывает/использует сам.
7. **Проверь:** фильтры/сорт/скролл/инлайн-правка/формы/экспорт/вставка из буфера работают; тема наследуется; контекст-фильтр применяется.

**Ретрофит существующего места:** замени ТОЛЬКО табличную часть; сохрани кастомные действия/формы и убедись, что бизнес-правила (статусы, согласование, генерация) по-прежнему доступны рядом с гридом.

> Кандидаты-списки в atex: orders, intake, warehouse, portal, slitter, sleeve-cutter.
> Не таблицы (вне замены): dashboards (виджеты), cut-map (визуализация), cut-planning (калькулятор), production-planning (очередь + редактор полос).
