...
Под списком сессий в `dir_admin` — секция «Задачи cron»: расписание, команда и пояснение
(комментарий строкой выше) из `crontab -l` пользователя сайта. Только просмотр: задачи заводит
администратор сервера.

- **Чьи задачи видны.** Владельцу базы — только задачи его базы: имя базы в строке запуска как
  `db=<база>` (`b24ig.php?db=…`, `--db=…`), первый сегмент адреса `https://сервер/<база>/…` или папка
  `templates/custom/<база>/`, `download/<база>/`. Поиск «отдельным словом», как у сессий, здесь не
  годится: база `ru` совпала бы с каждым `ideav.ru`, база `data` — с каждым `/var/www/www-root/data/`.
  `admin` видит все задачи.
- **Секреты** (`token=`, `*_TOKEN=`, `Authorization:`, `user:pass@`, `curl -u`, код вебхука Битрикса
  `/rest/<id>/<код>/`, cookie `idb_…`) маскируются до обрезки строки (300 символов).
- **Фигурные скобки** в команде безопасны: движок сам прячет их в подставленных значениях (`{` → `&#123;` до конца разбора), `{_global_.xsrf}` в строке cron выводится текстом — проверено меркой.
- Нет `exec` или у пользователя нет crontab — пустой список, а не ошибка.
- Логика — блок `# <crontab-dir-admin>` в обоих ядрах (совпадает дословно). Мерки:
  `experiments/crontab-dir-admin.test.php` (функции, отрисовка движком со старым и новым ядром) и
  `experiments/dir-admin-crontab.test.js` (инварианты шаблонов, в гейте).
