...
- **Список и номера (#4612):** в списке только незакрытые задания; выполненные и
  ...
  **Порядок списка — ПЛАНОВЫЙ** (по времени планового старта), и он же даёт **№ задания**:
  ...
  PPC-CORES черная 1" длина 1 метр · план 45 шт»), под ней мелким служебное («старт 08:00 ·
  факт · ширина рулона»); посередине кнопки **✓ Готово / Пропустить**, справа — счёт
  метровых заготовок. У закрытого задания кнопок нет (зелёный фон — «Готово», серый —
  «Пропущена»), и справа стои́т бейдж статуса: иначе непонятно, чем оно кончилось. Когда
  закрыты все задания дня, на месте списка — приглашение включить показ выполненных.
- **Что режем и сколько палок (#4786).** Заказ, название втулки и план идут в главную строку
  карточки основным цветом 16px: их втулкорез читает с планшета стоя, а прежние приглушённые
  14px не читались. Бейдж статуса **«В работе»/«Ожидает» снят** — состояние живого задания и
  так видно по кнопкам «Готово»/«Пропустить». Его место занял счёт заготовок: втулку, у
  записи которой в справочнике «Диаметр втулки» **снята галка «Готовые»**, режут из метровых
  палок, и на карточке стои́т **«К-во: N»** (подпись по решению заказчика), где
  **N = ⌈план × ширина рулона × 1.1 / 1000⌉** (решение заказчика 18.08.2026: палка длиной
  1000 мм режется по ширине рулона, 1.1 — запас на рез и брак; в подсказке бейджа — сама
  арифметика). Втулка с галкой «Готовые» приходит нарезанной — у неё бейджа нет вовсе.
  Источник — колонки отчёта `order_no`, `sleeve`, `position_width`, `sleeve_ready`
  (§14 в `integram-reports.md`). Колонок в отчёте нет → карточка показывает, что может,
  палки не считаются, и пульт называет недостающие колонки плашкой над списком.
