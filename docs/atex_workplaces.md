...
- **Задание на втулки только для «сырых» диаметров** (#3321): втулки хранятся в
  справочнике `Диаметр втулки` (на него ссылается позиция заказа реквизитом
  «Диаметр втулки»); у справочника есть поле «Готовые» (тип 11 — галочка). Если
  «Готовые» отмечено (значение непустое), этот тип втулок уже нарезан и задание
  на втулки создавать не нужно. Рабочее место загружает справочник
  (`loadSleeveDiameters`, резолв таблицы и колонки по имени) в карту
  `sleeveReadyDiameterIds`, а `positionSleeveTasksForLayout` пропускает позиции,
  чей диаметр «готов». Чистые функции `sleeveDiameterIdFromRow`,
  `sleeveDiameterIsReady` покрыты тестом `experiments/atex-production-planning.test.js`.
- **Дата «Нарезка втулок»** (#3321): в панели фильтров очереди между «Дата плана»
  и «Статус» добавлен выбор даты «Нарезка втулок». По умолчанию — на день раньше
  «Даты плана»; при смене «Даты плана» дата нарезки сдвигается на ту же разницу
  дней, ручная правка сохраняет смещение (чистые `shiftIsoDate`, `isoDateDiffDays`,
  `sleeveCutDateForPlan`). Дата пишется в реквизит «Нарезка втулок» (тип 9, ISO
  `YYYY-MM-DD`) создаваемых заданий на втулки, если такой реквизит есть в таблице
  `Задание на втулки` (резолв по имени, иначе пропускается). **Зависимость от
  схемы:** реквизит «Нарезка втулок» нужно добавить в таблицу `Задание на втулки`
  на сервере, чтобы дата сохранялась.
- **Список заданий на втулки** в `.atex-pp-link` под «Связанные позиции» (#3321):
  для каждой связанной позиции выводятся её задания из подчинённой `Задание на
  втулки` (`loadSleeveTasks` → `sleeveTasksByPosition`; строка собирается чистой
  `formatSleeveTaskLine`: диаметр, кол-во, факт, дата нарезки, статус, втулкорез).
