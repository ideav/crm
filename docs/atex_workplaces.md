...
- **Следующие задания (#4929):** ниже списка — блок «Следующие задания», по одной карточке
  ...
  `FR_packer_no={номер места}` и `FR_task=>{полночь сегодня}` (вчерашние так и не
  резанные задания кандидатами не становятся, сегодняшние опоздавшие остаются). Задания
