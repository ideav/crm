...
- **Реализация:** шаблон `templates/atex/orders.html`, логика
  ...
  резолвятся из `metadata/{tableId}` по имени (id таблиц 107/108 — в data-атрибутах
  корня `#atex-orders-app`), поэтому код переживает пересборку базы. Тест:
...
- **Реализация:** шаблон `templates/atex/slitter.html`, логика
  ...
  `experiments/atex-slitter.test.js` (35 проверок); браузерный стенд —
...
- **Реализация:** шаблон `templates/atex/cut-map.html` (URL `/atex/cut-map`),
  ...
  ID таблиц/реквизитов резолвятся по именам из `metadata?JSON`. Тесты:
...
- **Реализация:** шаблон `templates/atex/warehouse.html`, логика
  ...
  Реквизиты резолвятся из `metadata/{tableId}` по имени (id таблиц 113/109/110 —
  в data-атрибутах корня `#atex-warehouse-app`), поэтому код переживает
  пересборку базы. Первая колонка `Дата прихода` (DATETIME) не передаётся при
...
- **Таблицы (чтение):** `Заказ`, `Производственная резка`, `Партия ГП`,
  `Партия сырья`.
- **API:** `object/{typeId}/?_count=` для счётчиков, постранично
...
- **Реализация:** шаблон `templates/atex/dashboards.html` (URL `/atex/dashboards`),
  ...
  резолвятся из `metadata?JSON` по именам, поэтому код переживает пересборку базы.
...
- **Реализация:** шаблон `templates/atex/portal.html` (URL `/atex/portal`),
  ...
  заказы фильтруются по ссылке «Клиент» (реквизиты резолвятся из metadata по
  имени, поэтому код переживает пересборку базы). Без сопоставленного клиента
