...
- **Что видит упаковщик:** задание попадает в список, **как только по нему сделана первая
  ...
  место не выбрано, отчёт не запрашивается вовсе (#4681). Карточка: слева **крупный номер
...
- **Отчёт `packer`:** `task` (главное значение задания, Unix) и `task_id` (abn_ID),
  ...
  `notes` («Примечание»), `events` (COUNT событий смены), `packer_no` (упаковочное место
  позиции — по нему фильтр `FR_packer_no`). Скрытая колонка «Тип события» с
...
- **Реализация:** шаблон `templates/atex/packer.html` (URL `/atex/packer`), логика
  ...
  незарегистрированного планшета `packer.js` не грузится вовсе.
