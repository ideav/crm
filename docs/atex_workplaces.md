...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  не менялся — запроса нет (#4933 п.3). При завершении резки записываются длины,
  счётчик и число резок — уже фактические. Расход и списание без номера не сохраняются;
...
- **Что видит упаковщик:** задание попадает в список, **как только по нему сделана первая
  ...
  (integram-reports.md §15); их бывает несколько — все через «, ». Обе плашки бывают
