...
- **Поля «Длина, м» и «Остаток, м»** (исх. #52): при оприходовании кладовщик
  ...
  подставляется из справочника `Вид сырья.Длина рулона, м` (`calc.defaultLength`).
...
- **«Остаток, м» партии** (исх. #52): при сохранении показаний счётчика поле
  ...
  возвращается обратно (`calc.applyLinearRemainder`).
- **«Брак, м» и «Брак, м²»** (исх. #52): оператор вводит «Брак, м»;
  «Брак, м²» вычисляется автоматически: `Брак,м × Вид сырья.Ширина, мм / 1000`
  (`calc.defectM2`). Оба значения записываются в запись `Производственная резка`
  через `_m_set`.
- **«Фото брака»** (исх. #52): файл загружается через multipart-запрос и
  ...
  формируется как `t{reqId}` (`calc.photoFieldKey`).
