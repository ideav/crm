...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  «Счётчик кон.» предыдущего); на задании без записей номер подставляется из памяти
  планшета по виду сырья (localStorage). При завершении резки записываются длины,
...
- **API:** `_m_set/{резкаId}` (статус, счётчики, погонаж); джамбо —
  `_m_set/{id записи «Номер джамбо»}` (автосохранение и накопление отметками) и
