...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  — вкладка активной записи: «Счётчик нач./кон.», расход и поля
  ...
  atex_data_schema.md §6.7). Ввод уходит автосохранением при выходе из ячеек; отметка
  ...
  с какого джамбо. При завершении резки записываются длины и счётчик, а сумма резок
