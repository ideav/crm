...
- **Одна плашка на заказ (#4918):** заказ бывает разбит на несколько заданий — упаковщик
  ...
  сливаются — каждая живёт одиночной плашкой. **Выполненные и невыполненные позиции
...
- **Количество и отметка (#4680, #4918):** количество меняют **кликом по самому
  ...
  `qty_fact`, а если его нет — `qty`; у слитой плашки — **общая сумма заказа**, и правка
  меняет именно её: **разница с текущей суммой целиком уходит в последнюю неупакованную
  позицию — и может получиться отрицательной** (решение заказчика). Поправил относительно
  подставленного — **обязательно примечание**, оно уезжает в последнюю неупакованную
  позицию. Кнопка **«Упаковано» ничего не спрашивает**: она фиксирует упаковку тем
