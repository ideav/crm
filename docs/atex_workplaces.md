...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  несколько джамбо — над сеткой показаний закладки (номер каждого джамбо + «+»), поля
  «Номер джамбо / Рабочий расход / К списанию / Брак, м / Брак, шт / Фото брака» читают
  и пишут запись АКТИВНОГО джамбо (таблица «Номер джамбо», 82374, up = задание, схема —
  ...
  полях сразу накопленное. «+» добавляет джамбо («Счётчик нач.» нового = «Счётчик кон.»
  предыдущего); на задании без записей номер подставляется из памяти планшета по виду
  сырья (localStorage). При завершении резки записываются длины, счётчик и число резок —
  уже фактические. Расход и списание без номера не сохраняются; записи видит упаковщик
  (отчёт `task_jumbo`). Почему «Погонаж факт» не кратен метражу×резкам: это разница
  счётчиков — фактическое потребление джамбо (резка + рабочий расход + к списанию +
  брак), а не плановый метраж.
