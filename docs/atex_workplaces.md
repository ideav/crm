...
- **Реализация:** шаблон `templates/atex/orders.html`, логика
  ...
  и id таблиц резолвятся из `metadata?JSON` по имени, поэтому код переживает
...
- **Реализация:** шаблон `templates/atex/warehouse.html`, логика
  ...
  Реквизиты и id таблиц резолвятся из `metadata?JSON` по имени, поэтому код
...
- **Реализация:** шаблон `templates/atex/portal.html` (URL `/atex/portal`),
  ...
  `metadata?JSON` по имени, поэтому код переживает пересборку базы). Без сопоставленного клиента
