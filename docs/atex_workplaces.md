...
| 3 | Планирование производства | Диспетчер | Производственная резка, Полоса, Обеспечение, Позиция заказа, Слиттер, Партия сырья, Максимальный запас | [#2913](https://github.com/ideav/crm/issues/2913) |
...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  факт» = «Кол-во полос» × факт. проходов (погонаж факт ÷ метраж прогона, фолбэк
  на план) — фактически произведённые рулоны.
...
- **Действия:** калькулятор-визуализатор. По выбранному Виду сырья (ширина
  ...
  подбирает раскладку ножей с **минимальным отходом** и показывает, сколько
  рулонов получится **максимально близко к желаемому**: количество резок
  (проходов джамбо), полосы за резку и отход. Результат визуализируется
  полосой-картой (как в «Карте раскроя» 3.7), таблицей по ширинам и сводкой.
  Только чтение — ничего в схему не пишет (ср. упразднённые 3.2 и 3.11).
- **Таблицы (чтение):** `Вид сырья` — для поиска по справочнику и автоподстановки
  ширины («Ширина, мм») и длины рулона («Длина рулона, м»). Поля остаются
  редактируемыми, так что можно считать и без выбора сырья.
- **API:** единственный запрос данных — `object/{Вид сырья}/?JSON_OBJ&LIMIT=0,1000`
  для выпадающего списка с поиском (`AtexRefSearch`). Метаданные — `GET /{db}/metadata`;
  id таблиц/реквизитов резолвятся по именам, поэтому код переживает пересборку базы.
- **Модель расчёта:** джамбо шириной *W* режется вдоль на полосы; один проход
  (одна «резка») даёт набор полос по числу ножей. Желаемые количества сводятся к
  пропорции через НОД, в *W* укладывается максимум пропорциональных наборов
  (назначение «Заказ»), остаток добирается теми же ширинами для минимизации
  отхода (избыток сверх заказа → «Склад»). `Отход, мм` = *W* − Σ(ширина ×
  количество). Число резок *P* ≈ НОД/наборов, чтобы итог по рулонам был ближе
  всего к желаемому. Канонический пример: вход 880 → 60×14 + 40×1, отход 0.
- **Приёмка:** выбор сырья подставляет ширину/длину; кнопка «Рассчитать»
  показывает раскладку одной резки (цветная полоса Заказ/Склад/Отход), таблицу
  «ширина → желаемо / полос за резку / получится / Δ» и сводку (кол-во резок,
  полос за резку, итого рулонов, отход на резку и общий отход в м²). Ширины шире
  джамбо помечаются как непомещающиеся, а не отбрасываются молча.
- **Реализация:** шаблон `templates/atex/cut-optimizer.html` (URL
  `/atex/cut-optimizer`), логика `download/atex/js/cut-optimizer.js`, стили
  `download/atex/css/cut-optimizer.css`. Чистое ядро (`core.computePlan` — НОД,
  пропорция, добор остатка `fillRemainder`, сегменты-ножи, отход, отклонение)
  вынесено для модульных тестов. Тесты: `experiments/atex-cut-optimizer.test.js`
  (29 проверок), стенд `experiments/atex-cut-optimizer.fixture.html`. Пункт меню
  `cut-optimizer` — у ролей Администратор и Диспетчер (`docs/atex_menu.json`).
