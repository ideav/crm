...
- **Таблицы (запись, только «В заказ»):** `Заказ` (создаётся, если номер новый) и
  по одной `Позиция заказа` на каждую ширину (Вид сырья, Ширина в заказе =
...
- **API:** `GET /{db}/metadata` (id таблиц/реквизитов резолвятся по именам — код
  ...
  и `_m_new/{Позиция заказа}?up={orderId}` с токеном XSRF.
- **Фактическая ширина (#3474):** пользователь задаёт **номинал** («Ширина в
  ...
  втулке здесь не применяются — калькулятор знает только джамбо). Раскрой и отход
  считаются по фактической ширине; в Позицию заказа пишется номинал — планирование
...
- **Приёмка:** выбор сырья подставляет ширину/длину; «Рассчитать» показывает до
  ...
  заказе / желаемо / получится / Δ» и сводку с выделенными цветом главными
