...
- **Количество и отметка (#4680, #4918, #4999):** количество меняют **кликом по самому
  ...
  подставленного — **обязательно примечание**. Кнопка **«Упаковано» ничего не спрашивает**: она фиксирует упаковку тем
  количеством, **которое видно в карточке**, — на слитой плашке **отмечает ВСЕ позиции
  заказа, каждую Партию ГП своим количеством** (запись по-прежнему позиционная, поэтому
  «Дэшборд отклонений» РМ production-planning — Σ «Кол-во факт» против Σ «Упаковано шт»
  по заданию — остаётся согласованным). У неупакованной позиции правка живёт в модели
...
- **Реализация:** шаблон `templates/atex/packer.html` (URL `/atex/packer`), логика
  ...
  `experiments/atex-packer-5003-autorefresh.test.js`. Пункт меню `packer` — `docs/atex_menu.json`.
