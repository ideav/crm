...
- **Данные, которые рабочее место не чинит молча:**
  - карточка втулки, у которой количество лежит в старых реквизитах остатка, а
    ...
    перенос — `docs/scripts/migrate_sleeve_qty_4655.py` (заглушки без склада
    переносятся только ключом `--stubs`: их «Остаток, м» читает отчёт
    `sleeve_batches_active`, по которому планировщик подбирает партию втулок);
