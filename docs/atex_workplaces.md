...
- **Кнопка «Сгенерировать резки»** (исх. #52, эпик «упразднение Типа резки», F3):
  ...
  нативный `confirm`): необеспеченные позиции группируются по сырью, типу намотки
  и длине; для каждого профиля ядро **`cut-layout.planLayouts`** (F1) пакует
  ...
  (`positionsCovered` → резка). Источники: `positions_list`
