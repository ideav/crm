...
- **Кнопка «Сгенерировать резки»** (исх. #52, эпик «упразднение Типа резки», F3):
  ...
  раскладкой полос (Типов резки больше нет). После подтверждения (модалка, не
  нативный `confirm`): необеспеченные позиции группируются по сырью; для каждого
  сырья ядро **`cut-layout.planLayouts`** (F1) пакует ширины заказанных позиций в
  ширину джамбо (= Вид сырья «Ширина, мм») и добирает остаток **ходовыми**
  (`preferable_widths`, Популярность = `position_qty_sum`), объединяя позиции в
  окне «Срок изготовления ± 3 дня». На каждую раскладку последовательно: `_m_new`
  Производственной резки (Статус «Запланирована»; станок не из стоп-листа с
  балансировкой `pickSlitter`; старейшая партия `pickBatchFIFO`) → `_m_new`
  подчинённых «Полос» (Ширина/Количество/Назначение) → `_m_new` «Обеспечений»
  (`positionsCovered` → резка). Источники: `positions_list` (+`position_due_date`),
  `preferable_widths`, ширина джамбо из «Вид сырья», обеспечения из `cut_planning`.
