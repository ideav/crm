...
- **Время на резку — только намотка (#4832).** Метрика «Время на резку» (#3744) считает
  «Всего резок» × время намотки рулона (нормы WIND_* из «Время операции, мин»);
  **наладку/переналадку в оценке нет** — решение заказчика («убирали это»): настройка
  станка — забота плана, а не менеджерской оценки. Норм в таблице нет (или длина не
  задана) — метрика честно показывает «—», а не выдуманные минуты.
