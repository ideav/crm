...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  #4914: в показаниях счётчика — расход **джамбо**. Джамбо хранится одной партией
  (16–20 км), и помимо нарезанного метража из него тратится протяжка перед резкой
  (**Рабочий расход, м**) и остаток под утиль (**К списанию, м**). На задании бывает
  несколько джамбо (#4916) — НАД панелью показаний строка корешков-закладок: корешок
  на каждую запись, активный корешок — само поле номера (редактируется), остальные
  записи открываются кликом по своему корешку, правее «+ Джамбо». Панель под корешками
  — вкладка активной записи: «Счётчик нач./кон.», «Погонаж факт» и поля
  «Рабочий расход / К списанию / Брак, м / Брак, шт / Фото брака» читают и пишут запись
  АКТИВНОГО джамбо (таблица «Номер джамбо», 82374, up = задание, схема —
  atex_data_schema.md §6.7). #5010: счётчики и погонаж — ЗНАЧЕНИЯ ЭТОЙ ЗАПИСИ, а не
  задания: «Погонаж» записи = «Счётчик нач.» − «Счётчик кон.», отметка прохода ведёт
  счётчики активной записи (пустое начало наследует счётчик резки, кон. сдвигается на
  резки×метраж и ввод), правка «Счётчика нач.» в записи смещает её «кон.» на ту же
  дельту. Задание при нескольких джамбо своей парой счётчиков показать рулон не может
  (сумма погонажа всех записей к остатку партии первого неприменима), поэтому
  заданиевые «Счётчик нач./кон.» и «Погонаж факт» оператору не показываются — их ведут
  отметки для партии/отчётов. Остаток партии сводится с «кон.» активной записи (а не с
  заданиевым вычислением). Ввод уходит автосохранением при выходе из ячеек; отметка
  прохода поля НЕ очищает — введённое прибавляется к накопленным значениям записи, и в
  полях сразу накопленное. «+ Джамбо» добавляет запись («Счётчик нач.» нового =
  «Счётчик кон.» предыдущего); на задании без записей номер джамбо НЕ подставляется —
  оператор вводит его руками (#4933 п.2). Номер существующей записи правится и после
  закрытия задания: номер — главное значение записи, пишется через `_m_save/{id}`
  с ключом `t82374` (kb/crud.md: `_m_set` главное значение молча игнорирует, #4906);
  не менялся — запроса нет (#4933 п.3). #5005: отметка прохода кладёт свои резки
  («Кол-во резок», 82378) в запись АКТИВНОГО джамбо — по записям видно, сколько резок
  с какого джамбо. При завершении резки записываются длины и счётчики записи (свои,
  отметками ведённые; пустое начало записи наследует счётчик резки — записи до #5010),
  а сумма резок
  по записям задания сверяется с фактом: недостача (отметки до ввода накопления)
  доносится в активную запись, накопленное общим числом задания не перезаписывается.
  Расход и списание без номера не сохраняются;
  записи видит упаковщик (отчёт `task_jumbo`). Почему «Погонаж факт» не кратен
  метражу×резкам: это разница счётчиков — фактическое потребление джамбо (резка +
  рабочий расход + к списанию + брак), а не плановый метраж.
...
- **API:** `_m_set/{резкаId}` (статус, счётчики, погонаж); джамбо —
  `_m_set/{id записи «Номер джамбо»}` (автосохранение и накопление отметками),
  `_m_save/{id записи}` (правка номера — главного значения, #4933 п.3) и
  `_m_new/82374?up={резкаId}` (первый ввод без записи и «+»); чтение записей —
  `report/task_jumbo?JSON_KV&FR_task_id={резкаId}`; расход —
...
- **Компактный экран пульта (#4783):** пульт занимает рабочую область целиком, и
  ...
  - **кнопки «Сохранить показания» нет** — запись уходит при выходе из ячейки
    ...
    предыдущей записи не теряется (повтор по её завершении), результат виден статусом
    в правом краю строки корешков джамбо (#4933 п.1).
  Тест: `experiments/atex-slitter-4783.test.js`.
- **Клавиатура планшета не уводит пульт (#4914).** Фокус в поле поднимал клавиатуру,
  браузер панорамировал документ и не возвращал его обратно — шапка уезжала, и поправить
  это можно было только перезагрузкой страницы. Теперь документ пульта не скроллится
  вовсе: для полноэкранных мест в `<head>` инжектируется `html, body { overflow: hidden }`
  (тот же механизм, что у решения «левого меню», #4783), а viewport-мета
  `templates/atex/main.html` несёт `interactive-widget=resizes-content` — клавиатура
  сжимает рабочую область вместо панорамирования. Прокручивается только `.atex-sl-layout`;
  смещение после закрытия клавиатуры сбрасывает сторож `visualViewport` в slitter.js.
- **Компактная шапка и скролл правой части (#4933 п.1/п.4).** Заголовок
  («номер заказа + Резка N из M»), спецификация и кнопки управления — строчные блоки
  ОДНОЙ строки шапки (`.atex-sl-headwrap` — flex-row с переносом); отдельной строки
  `.atex-sl-section-head` в панели показаний нет — статус сохранения живёт в строке
  корешков джамбо. Правая часть (`.atex-sl-main`) прокручивается сама
  (`overflow-y: auto`): в ландшафтном режиме планшета содержимое выше экрана, и без
  своего скролла нижние поля ввода были недостижимы (страничный скролл на широком
  экране выключен, #4914; на узком — у документа, #4924).
  Тест: `experiments/atex-slitter-4933.test.js`.
...
- **Раскладка читается с одного взгляда (#4785):**
  ...
  - **подсказок под полями нет** (`.atex-sl-hint`): остаток партии виден строкой партии
    внизу, брак ведётся в записи «Номер джамбо» (#4914) — пересчёты и предупреждения
    по ширине сырья оператору не нужны. Осталось одно сообщение — ответ на загрузку
    снимка («фото загружено»), иначе не видно, доехал ли он.
...
- **Номер заказа в задании (#4606):** в каждой карточке очереди и в шапке текущего
  ...
  уходит второй строкой (`.atex-sl-cut-line1`, формат — ниже, #4958); у задания без
  заказа в первой строке остаются № по порядку и статус. Источник —
...
- **Карточка очереди: статус в строке номера, позиция — как у диспетчера (#4958).**
  Карточка состоит из двух строк (`cutCardRows` в `slitter.js`, общие у заданий дня
  и у плашки будущих дней):
  - первая — маркер (№ по порядку либо «→»/«✓»), крупный номер заказа и **статус**
    (`.atex-sl-badge`, прижат к правому краю). Отдельным столбцом карточки статуса нет:
    в столбце он вставал по вертикали посередине, то есть напротив спецификации, и
    читался как подпись к ней;
  - вторая — позиция задания тем же порядком слов, каким она написана в плане
    диспетчера (`formatStripSummaryLine`, `production-planning/10-planning-engine.js`):
    **«{сырьё} {ширина} x {длина} {намотка} - {N} резок»**, например
    `MB 30 x 122 IN - 11 резок`. Оператор сверяет карточку с планом глазами, поэтому
    порядок частей там и тут один. Счётная форма слова согласуется (1 резка, 3 резки,
    11 резок). Неизвестная часть опускается, а не даёт прочерк: ширины нет у задания без
    обеспечения, намотки — у задания без позиции, хвоста «- N резок» — у задания-настройки.
  Ширина позиции приходит из того же отчёта `report/cut_planning`, что и номер заказа
  (колонка `cut_roller_width`, «Ширина, мм» позиции заказа) — отдельного запроса подпись
  не стоит, обе карты (`cutOrders` и `cutWidths`) собираются из одного ответа
  `loadCutOrders` и кэшируются на выбранный станок. Строк на задание столько, сколько у
  него обеспечений, и позиции бывают разной ширины — уникальные показываются через «/»
  (`MB 30/50 x 122 IN`). Тест: `experiments/atex-slitter-4958-queue-card.test.js`.
...
- **Фактическая ширина (#3474):** пользователь задаёт **номинал** («Ширина в
  ...
  фактическую с учётом условия по ширине джамбо (`j=910`/`j>1000`) и по диаметру втулки
  (`s=0.5`, #4957 — диаметр задаётся в форме). Условие `j=`
...
- **Втулка 0,5″: размерная сетка и втулочные полосы (#4957).** Калькулятор считает те же
  особенности втулки 0,5″, что и планирование производства (#3812,
  `docs/atex_production_planning_algorithm.md` «Шаг 2.0»):
  - у 0,5″ спрашивается ещё и **«Ширина втулки, мм»**. Варианты — ширины **готовых** втулок
    этого диаметра из справочника `Диаметр втулки` (реквизит «Ширина втулки, мм», а если он
    не заполнен — из названия записи: «Втулка картонная 0.5" ширина 110 мм» → 110); метровая
    палка вариантом не становится. Готовых втулок такого диаметра в справочнике нет — поля нет;
  - риббон **у́же 55 мм** на 0,5″ **не производится**: такая ширина убирается из расчёта, и
    плашка над планом её называет. Точки запаса у́же 55 мм при 0,5″ **не предлагаются** —
    нарезать впрок то, чего не делают, нельзя;
  - при **ширине втулки 110 мм** в резку добавляются **полосы 110 мм**: продуктовая ширина
    **55–57 мм → 2 полосы**, **63–64 мм → 1 полоса**, прочие (58–62, 65–70) — ни одной;
    ширина **> 70 мм** режется по обычному правилу втулки 1″. Диапазон считается по
    **номинальной** ширине заказа. Полосы занимают ширину джамбо **той же резки**
    (резервируются до укладки продукта), на рисунке раскроя у них свой цвет и ключ легенды,
    а в «Полос/резку» они посчитаны. Примеры заказчика: 891 → **55×12 + 110×2** и
    **64×12 + 110×1**;
  - ширина **110 мм, заданная в желаемых рулонах**, обеспечивается этими же полосами и не
    режется сверх них (как в планировании, #3872): в таблице её строка помечена
    «втулочные полосы», выпуск = полосы × проходов;
  - ширины **разных диапазонов** (например 55 и 63) в производстве идут **разными резками**
    (число втулочных полос — измерение профиля планирования), а калькулятор кладёт все ширины
    на одну карту. В таком задании полосы не добавляются, и плашка говорит об этом прямо.
- **Диаметр втулки в фактической ширине (#4957).** Правила `s=…` справочника `Фактическая
  ширина резки` (условие по диаметру втулки, напр. `s=0.5`) применяются: диаметр задан в форме
  и уходит в резолв вместе с шириной джамбо. Ни одно условие не выполнено — берётся номинал.
...
- **Приёмка:** выбор сырья подставляет ширину и показывает подходящие точки запаса;
  выбор втулки 0,5″ шириной 110 мм на задании 55×12 (вход 891) даёт раскрой
  **55×12 + 110×2** с отходом 11 мм, а ширина 50 мм в том же задании выбывает с плашкой
  «на втулке 0,5″ риббон у́же 55 мм не производится» (#4957);
...
- **Реализация:** шаблон `templates/atex/cut-optimizer.html` (URL
  ...
  `core.matchStockPoints` — отбор точек запаса; `core.isSleeveWidthProducible` и
  `core.sleeveCoreStripPlan` — правила втулки 0,5″, те же числа, что в
  `download/atex/js/production-planning/10-planning-engine.js`) вынесено
  ...
  `experiments/atex-cut-optimizer-4957-sleeve.test.js`,
...
- **Действия (#4658):** в правом верхнем углу — **упаковочное место**; оно выбирается один
  ...
  клику на само место (сменить). Ниже — список **плашек заказов** к упаковке **в порядке
  отчёта**: задания одного заказа схлопываются в одну плашку (#4918, ниже). Полностью
  упакованные заказы скрыты, пока не включён переключатель **«Показать упакованные (N)»**
  (N — число скрытых заказов; localStorage `atex-pk-show-packed`).
...
- **Одна плашка на заказ (#4918):** заказ бывает разбит на несколько заданий — упаковщик
  должен видеть, **сколько ему всего упаковать**. Поэтому список группируется по
  `order_no`, а не по заданию: на плашке заказа — **общее количество рулонов неупакованных
  позиций**, в мете — времена всех его заданий («задания 08:00, 14:30»), Σ план/факт по
  неупакованным, разные подписи роликов — отдельными строками, короб — по разным
  типоразмерам с суммой по каждому. Позиции без номера заказа чужие друг с другом не
  сливаются — каждая живёт одиночной плашкой. В резке бывает несколько размеров, поэтому
  **у каждой строки описания на слитой плашке — СВОЁ количество** (#4999, серый хвост
  « · N шт»; у упакованной позиции — записанное); у одиночной позиции количество одно —
  крупное, дублей в строке нет. **Выполненные и невыполненные позиции
  одного заказа — одна плашка «как невыполненная»** со статусом **«частично»**
  (янтарный бейдж `.atex-pk-badge.is-partial`); полностью упакованный заказ получает
  зелёный «упаковано» и прячась, показывает себя по переключателю.
- **Количество и отметка (#4680, #4918, #4999):** количество меняют **кликом по самому
  количеству** — крупное число в карточке нажимается и открывает модалку
  «Количество · заказ N» (количество + примечание). У одиночной позиции подставляется
  `qty_fact`, а если его нет — `qty`. Когда неупакованных позиций несколько
  (несколько размеров в резке), модалка — **«Количество по размерам · заказ N»: по полю
  на каждый размер** со своей подписью; примечание общее, обязательно при изменении хоть
  одного количества и уходит только изменённым позициям. Правка общей суммы — **«разница
  с текущей суммой целиком уходит в последнюю неупакованную позицию, может получиться
  отрицательной»** (решение заказчика по #4918) — остаётся для случая **одной**
  неупакованной позиции. Поправил относительно
  подставленного — **обязательно примечание**. Отметок две. **У каждой неупакованной
  строки слитой плашки — своя кнопка «Упаковано»** (#5011): она фиксирует упаковку
  **этой позиции** — той Партии ГП, что в строке стоит, тем количеством, которое видно
  у строки (с правкой и примечанием, если они стояли); у упакованной строки своей кнопки
  больше нет, у одиночной позиции строковой не нужно — крупная карточная пишет её же.
  Крупная кнопка **«Упаковано» в правом блоке карточки ничего не спрашивает и отмечает
  весь остаток** — все неупакованные позиции заказа, каждую Партию ГП своим количеством
  (запись позиционная, поэтому «Дэшборд отклонений» РМ production-planning — Σ «Кол-во
  факт» против Σ «Упаковано шт» по заданию — остаётся согласованным). Количества нет —
  в отчёте не было ни плана, ни факта: записи не происходит, оператору показывает
  сообщение и сразу открывается правка количества. У неупакованной позиции правка живёт в модели
  до отметки (карточка обводит поправленное количество), у упакованной — уходит в базу
  сразу, потому что откладывать её уже некуда; у упакованной группы правка открывается
  на последней позиции — туда же по правилу уходят корректировки. Позиция считается
  упакованной при **любой ненулевой записи «Упаковано шт», включая отрицательную** —
  иначе заказ с ушедшей в минус позицией возвращался бы в список с полным фактом.
- **Что видит упаковщик:** задание попадает в список, **как только по нему сделана первая
  ...
  места не увидит ни один упаковщик. Карточка: слева **крупный внутренний номер заказа**
  (`order_no`), а под ним мелко **«Заказ клиента»** (колонка `order` отчёта) — тот, что
  напечатан **на этикетке ролика** (#4688, #4912); у заказа без клиентского номера второй
  строки просто нет; с #4799 разрыв между этими двумя строками уменьшен — клиентский номер
  стал мельче, внутренний крупнее, но первый по-прежнему больше второго на любой ступени
  (`is-long`/`is-tiny`). Посередине привычная подпись ролика
  `MWR113L 110 х 600 IN втулка пластик серая Прозрачный + доп. втулка: Приклеить` — **лидер**
  (колонка `leader`, #4799) встаёт следом за втулкой, перед хвостом доп. втулки. Ниже одна
  строка меты — время задания, план/факт и акцентом в её хвосте **короб** («короб №165 ·
  по 24 шт · 5 коробов», #4665, span `.atex-pk-pack` внутри `.atex-pk-meta`, #4910).
  Последними в теле карточки — плашки по ширине содержимого (inline-flex, не растягиваются
  на всю колонку): **артикул** (колонка `art`, #4799, `.atex-pk-art`) и рядом **№ джамбо**
  (#4910, `.atex-pk-jumbo`) — номера джамбо задания из отчёта `task_jumbo`
  (integram-reports.md §15); их бывает несколько — все через «, ». #5005: если по записям
  хватает данных, плашка показывает количество по каждому джамбо — «qqq — 39 шт» =
  резки джамбо × штук в резке (факт позиции / Σ резок задания) − брак джамбо в шт;
  брак вычитается один раз на джамбо. Резок или факта нет — прежний список номеров.
  Обе плашки бывают
  пустыми, тогда карточка просто без соответствующей. Справа количество и кнопка.
  Заголовков заданий над карточками нет: плашка — сам заказ, времена его заданий стоят
  в её мете.
- **Следующие задания (#4929):** ниже списка — блок «Следующие задания», по одной карточке
  на станок места: самое раннее задание **сегодняшнего дня и позже**, по которому **ещё нет
  ни одной резки**. Оператор отмечает резку в конце работы, а ролики едут упаковщику сразу —
  блок даёт приготовить тару, наклейки и короба под то, что поедет следующим. Источник —
  отчёт `packer_next` (integram-reports.md §16): те же колонки, что у `packer`, плюс
  `slitter`/`slitter_id`, но **без** фильтра по событию «Резка»; запрашивается с
  `FR_packer_no={номер места}`. Окно «сегодня и позже» держит **внутренняя граница
  отчёта «>= [TODAY]» по часам сервера** — внешний фильтр даты клиент не шлёт:
  он перекрывал серверную границу, и на планшете со сбитой датой в «следующие»
  попадали задания двухнедельной давности (#5007: 5248/5235 с планом на 10–11.09
  показывались 25.09). Задания
  основного списка исключаются по `task_id` — по ним резка уже была. Карточка
  информационная: заказы задания, подписи роликов, станок, время задания
  (`taskWhenLabel`: сегодня — временем, иначе `ДД.ММ, ЧЧ:ММ`), Σ план и короба по
  типоразмерам; количества-кнопки и «Упаковано» нет — отмечать пока нечего, бейдж
  «следующее», рамка пунктиром (`.atex-pk-card.is-next`). Отчёта на базе нет или он не
  прочитался — блока просто нет, остальное рабочее место живёт как раньше.
- **Авто-обновление (#5003):** планшет открыт днями, а план перепланируется — задания
  исчезают из очереди, у оставшихся меняется время, и снимок на экране устаревает молча
  (issue #5003: на двух устройствах одного места — разные списки заданий). Данные
  перечитываются сами: по возврату на вкладку (`visibilitychange`), по фокусу окна и раз
  в 5 минут. Перечитывание отменяется, пока открыт диалог, есть несохранённая правка
  количества (её перечитывание стёрло бы) или предыдущая загрузка была меньше 30 секунд
  назад; у кнопки «Обновить» — подпись свежести «данные на ЧЧ:ММ». Без упаковочного
  места данные не запрашиваются вовсе (#4681).
- **Часы устройства (#5007):** серверное время читается из заголовка `Date` каждого
  ответа; если часы планшета разошлись с сервером больше чем на 10 минут, в шапке —
  предупреждение «часы устройства не совпадают с сервером» (`.atex-pk-clock-warn`).
  Сбитая дата не двигает очередь (её держит сервер, см. «Следующие задания»), но врёт
  в подписях заданий («сегодня» не тот день) и во времени записываемых событий
  упаковки — лечится авто-датой на устройстве.
...
- **API:** список — `report/packer?JSON_KV&LIMIT=0,5000&FR_packer_no={номер места}`;
  ...
  («Упаковано шт», «Примечание») и `_m_new/{Событие смены}?up=1`; «Упаковано» на слитой
  плашке пишет эту пару на **каждую неупакованную Партию ГП заказа** — последовательно,
  своим количеством и своим примечанием (#4918). Отчёт после записи не
...
- **Приёмка:** первый вход спрашивает упаковочное место (после выбора запрос к таблице
  ...
  «Упаковка» без станка; у слитой плашки строковое «Упаковано» (#5011) пишет только
  свою позицию — плашка вешает «частично», карточная кнопка дописывает остаток; после
  перезагрузки упакованные позиции видны упакованными;
  открытый экран перечитывает данные сам — штамп «данные на ЧЧ:ММ» у «Обновить»
  двигается без ручного нажатия (#5003).
- **Реализация:** шаблон `templates/atex/packer.html` (URL `/atex/packer`), логика
  ...
  `experiments/atex-packer.test.js`, стражи авто-обновления (#5003) —
  `experiments/atex-packer-5003-autorefresh.test.js`, по-позиционная отметка
  строк слитой плашки (#5011) — `experiments/atex-packer-5011-per-position-pack.test.js`.
  Пункт меню `packer` — `docs/atex_menu.json`.
...
**Код планшета уходит администратору сам (#4944).** Незарегистрированный планшет
пишет свой код в таблицу **«Планшет-кандидат»** (ateh, id 804338, `"granted":"WRITE"`
у оператора, реквизитов нет — только первая колонка). Диспетчер берёт код оттуда и
переносит в «Планшет»; на экране код по-прежнему виден — его можно продиктовать.

- **Когда пишется.** Только когда устройства нет в «Планшете»: экран «Устройство не
  зарегистрировано» сторожа и экран «Рабочее место не настроено» входа оператора.
  Опознанный планшет, отказ по роли (#4868) и незаполненная настройка уже
  зарегистрированного планшета в «Кандидата» не пишут.
- **Что пишется.** В таблице одна запись. Записи нет — создаётся:
  `_m_new/804338?JSON&up=1` с `t804338={код}`. Запись есть с другим кодом — код
  переписывается поверх: `_m_save/804338-записи?JSON` с тем же `t804338={код}`
  (первая колонка правится только `_m_save`, `_m_set` её молча не меняет, #4906).
  Тот же код уже записан — запроса нет.
- **Что видит оператор.** Строка под кодом: «Код передан администратору: он записан
  в таблицу „Планшет-кандидат“». Сервер отказал или таблицы в базе нет — код остаётся
  на экране, при отказе строка называет ошибку.
- **Гранты.** Оператору «Планшет-кандидат» выдан **WRITE** (запись 804339 таблицы 116),
  «Планшет» — READ (673808). WRITE покрывает и чтение: `Check_Grant` засчитывает
  WRITE на любой запрошенный уровень (`index.php`, «Requested or WRITE (higher)»),
  поэтому список записей кандидата оператору тоже открыт.
- **Почему `_m_save`, а не `_m_set`.** На боевой ateh `_m_set/{запись}` с `t804338`
  отвечает `[{"error":"У вас нет доступа к реквизиту объекта …"}]` и ничего не пишет:
  реквизита, равного id самой таблицы, не существует. `_m_save/{запись}` с тем же
  полем значение перезаписывает.
- **Реализация:** `core.candidateWrite` / `core.publishCandidate` в
  `download/atex/js/pad-guard.js`, экран входа — `announceCode` в
  `download/atex/js/pad-home.js` (токен XSRF приходит атрибутом `data-pad-xsrf` из
  `templates/atex/main.html`), стиль `.atex-pad-status` в `atex-brand.css`. Тест:
  `experiments/atex-pad-4944-candidate.test.js`.
