...
- **Реализация:** шаблон `templates/atex/slitter.html`, логика
  ...
  планшета** — см. §3.13: с незарегистрированного планшета `slitter.js` не
  грузится вовсе.
...
- **Реализация:** шаблон `templates/atex/sleeve-cutter.html`, логика
  ...
  планшета** — см. §3.13: с незарегистрированного планшета `sleeve-cutter.js` не
  грузится вовсе.
...
### 3.13 Допуск планшета к пультам оператора (#4666)

Пульты оператора открываются **только с зарегистрированного планшета**. Планшеты
...
- **Как устроено.** Токен устройства лежит в `localStorage` под ключом `atehPad`.
  При входе в пульт сторож `download/atex/js/pad-guard.js` берёт этот токен и ищет
...
- **Пульт не успевает отрисоваться, потому что его код не грузится.** Сторож
  ...
  ```
  Флаг внутри самого пульта такой гарантии не даёт: пульт стартует по
  `DOMContentLoaded`, то есть раньше, чем вернётся ответ сервера. Пульты
  переживают позднюю загрузку — их бутстрап проверяет `document.readyState` и при
  готовом документе зовёт `init()` сразу (`slitter.js`, `sleeve-cutter.js`).
...
- **Право на чтение обязательно.** Роль «Оператор» должна иметь грант **READ** на
  таблицу «Планшет» — иначе `object/673803/…` отвечает отказом и пульт не
  открывается ни с каким токеном.
- **Реализация:** `download/atex/js/pad-guard.js`, стили `.atex-pad-*` в
  ...
  и `templates/atex/sleeve-cutter.html`. Тесты:
  `experiments/atex-pad-guard.test.js` (чистые функции),
  `experiments/atex-pad-guard-boot.test.js` (загрузка пульта, отказ, регистрация).
  Пульт упаковщика (`packer.html`) в репозитории ещё не заведён — когда появится,
  сторож подключается к нему теми же двумя атрибутами.
