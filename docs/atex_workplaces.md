...
- **Действия (#3869, перестроено по образцу Пульта слиттера):** смену не открываем.
  Сверху — выбор втулкореза (запоминается в localStorage); ниже — список заданий на
  втулки этого втулкореза. По заданию: **✓ Готово** (статус = «Готово» + «Кол-во факт» =
  «К-во план») или **Пропустить** (статус = «Пропущена»); сверху **✓✓ Закрыть все** —
  пометить все незавершённые задания «Готово» (с подтверждением). Завершённые/пропущенные
  задания остаются в списке (бейдж статуса, без кнопок). В партию ничего не пишем.
- **Таблицы:** `Задание на втулки` (alias таблицы `К-во план`) ссылается на `Втулкорез`
  (реквизит `Втулкорез`). Главное значение (первая колонка `К-во план`) — плановое
  количество втулок ([#3159](https://github.com/ideav/crm/issues/3159)). Список заданий
  фильтруется по выбранному втулкорезу (реквизит `Втулкорез`).
- **API:** справочник втулкорезов — `object/{Втулкорез}/?JSON_OBJ`; задания —
  `object/{Задание на втулки}/?JSON_OBJ` (все, фильтр по втулкорезу — на клиенте,
  дедик-отчёта нет); отметка статуса/факта — `_m_set/{заданиеId}` (главное значение и
  `up` не трогаем).
- **Приёмка:** без выбора втулкореза — приглашение выбрать; список = задания втулкореза,
  активные выше завершённых; «Готово»/«Пропустить»/«Закрыть все» меняют статус; в партию
  записи нет.
