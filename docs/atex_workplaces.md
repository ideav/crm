...
- **Реализация:** шаблон `templates/atex/orders.html`, логика
  ...
  и id таблиц резолвятся из `metadata?JSON=1` по имени, поэтому код переживает
...
- **Реализация:** шаблон `templates/atex/production-planning.html` (URL
  ...
  `metadata?JSON=1` по имени, поэтому код переживает пересборку базы: резка —
...
- **Реализация:** шаблон `templates/atex/intake.html`, логика
  ...
  резолвятся из `metadata?JSON=1` по имени, поэтому код переживает пересборку
...
- **Реализация:** шаблон `templates/atex/slitter.html`, логика
  ...
  таблиц и реквизитов резолвятся из `metadata?JSON=1` по имени, поэтому код
...
- **Реализация:** шаблон `templates/atex/sleeve-cutter.html`, логика
  ...
  `/atex/sleeve-cutter`). Id таблиц и реквизитов резолвятся из `metadata?JSON=1`
...
- **Реализация:** шаблон `templates/atex/cut-map.html` (URL `/atex/cut-map`),
  ...
  ID таблиц/реквизитов резолвятся по именам из `metadata?JSON=1`. Тесты:
...
- **Реализация:** шаблон `templates/atex/warehouse.html`, логика
  ...
  Реквизиты и id таблиц резолвятся из `metadata?JSON=1` по имени, поэтому код
...
- **Реализация:** шаблон `templates/atex/dashboards.html` (URL `/atex/dashboards`),
  ...
  резолвятся из `metadata?JSON=1` по именам, поэтому код переживает пересборку базы.
...
- **Реализация:** шаблон `templates/atex/portal.html` (URL `/atex/portal`),
  ...
  `metadata?JSON=1` по имени, поэтому код переживает пересборку базы). Без сопоставленного клиента
