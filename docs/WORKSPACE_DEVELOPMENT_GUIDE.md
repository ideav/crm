...
> [`integram-app-workflow.md`](https://github.com/ideav/slitterplan/blob/master/docs/integram-app-workflow.md)
> (репозиторий ideav/slitterplan). В нём, по замечанию из issue #2797, много ошибок,
> поэтому настоящее руководство собрано заново из проверяемых источников, а
> корректность каждого раздела проверяется отдельной задачей (см. таблицу ниже).
> Ссылка приведена для сверки и отслеживания происхождения правил (issue #2808).
...
([`integram-app-workflow.md`](https://github.com/ideav/slitterplan/blob/master/docs/integram-app-workflow.md))
добавлена по issue #2808. Источники правил — код репозитория и закрытые тикеты
ideav/crm; см. ссылки `issue #NNN` и `file:line` в каждом разделе.*
