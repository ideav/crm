...
- **Комбинированная версия в `main.html`** (#4043): `?{_global_.version}.<N>` — общая (ядро)
  ...
  все базы) ИЛИ локальный `.<N>` (обновит одну). Так уже сделаны `templates/main.html` и
  `templates/ru/main.html`. Прочие `main.html` (`atex/`, `my/`, `sportzania/`, `db…`) свои
  `/js/*`/`/css/*` пока грузят без версии — техдолг (бамп `VERSION` их не обновит).
