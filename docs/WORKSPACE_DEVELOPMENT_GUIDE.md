...
- **Общая версия — `VERSION` в `index.php`** (`define("VERSION", …)`, экспортируется как
  `{_global_.version}`). Это единственная ручка сброса кэша для ядра: **бить при любых правках
  js/css ядра** (#3418) — один бамп обновляет **все** ассеты, у которых в URL стоит
  `?{_global_.version}`. Отдельно в каждом файле счётчик руками не правится.
- **Комбинированная версия в `main.html`** (#4043): `?{_global_.version}.<N>` — общая (ядро)
  плюс **локальный** суффикс `.<N>`, зашитый в самом `main.html`. Локальный суффикс бьётся прямо
  в `templates/custom/<z>/main.html` конкретной базы — чтобы сбросить кэш **одной** базы, не трогая
  ядро (у каждой базы суффикс свой). Достаточно поменять **одно из двух**: общий `VERSION` (обновит
  все базы) ИЛИ локальный `.<N>` (обновит одну). Комбинированную версию несут **все шесть**
  `main.html`: базовые (`templates/main.html`, `templates/ru/main.html`) и кастомные
  (`atex/`, `my/`, `sportzania/`, `db1341756924ugo/`). **Правило:** при добавлении в любой
  `main.html` нашего (first-party) `/js/*`/`/css/*` — сразу проставляй `?{_global_.version}.<N>`;
  ядро об этом не заботится, суффикс `.<N>` бьёт сам автор правки. Вендор с версией в имени
  (`bootstrap4.5.2.min.js`, `jquery3.1.1.min.js`, `popper1.16.1.min.js`, `moment.min.js`,
  `primeicons`) суффикс **не** получает — версионируется своим путём. `brand.css` отдаётся через
  `/download/{_global_.z}/…` и уже несёт `?{_global_.version}` — трогать не нужно. Гард против
  регресса: `experiments/templates-main-version-4043.test.js`.
