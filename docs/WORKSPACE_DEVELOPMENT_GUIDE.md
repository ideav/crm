...
Каждый раздел проверяется отдельной задачей (см. issue #2797). Таблица заполняется
номерами трекинг-задач после их создания.

| # | Раздел | Трекинг-задача |
|---|--------|----------------|
| 1 | Шаблонизатор и переменные `{_global_.*}` | _см. issue #2797_ |
| 2 | Подключение CSS/JS и версионирование ресурсов | _см. issue #2797_ |
| 3 | Вызовы API `_m_*` | _см. issue #2797_ |
| 4 | Токен XSRF и хелпер `intApi` | _см. issue #2797_ |
| 5 | Чтение данных из отчётов | _см. issue #2797_ |
| 6 | Чтение данных из таблиц | _см. issue #2797_ |
| 7 | Компонент IntegramTable | _см. issue #2797_ |
| 8 | Модальные окна и запрет `alert`/`confirm`/`prompt` | _см. issue #2797_ |
| 9 | Подсказки для пользователя (`js/hints.js`) | _см. issue #2797_ |
...
*Документ создан по issue #2797. Источники правил — код репозитория и закрытые тикеты
