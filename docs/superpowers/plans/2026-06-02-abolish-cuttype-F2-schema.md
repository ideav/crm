...
## Task 0: Бэкап, токен/xsrf и снимок состояния

**Files:** none (live).

- [ ] **Step 0: Полный бэкап БД ПЕРЕД необратимыми шагами.** Обязательно по умолчанию (см. [[feedback_integram_backup_before_irreversible]] / integram-app-workflow «Принцип безопасности данных»). В этом прогоне Андрей подтвердил, что бэкап уже сделан и данные не ценны — исключение по явному разрешению.
