...
6. `106-partiya-syrya.csv` -> таблица `Партия сырья` ({counts['batches']} строк).
   ...
   Дата прихода оставлена пустой, потому что в источнике нет дат партий.
...
def main() -> None:
    ...
    write_csv(out_dir / "106-partiya-syrya.csv", ["Партия сырья", "Вид сырья", "Дата прихода", "Получено, м²", "Остаток, м²"], batch_rows)
