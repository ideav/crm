...
# issue #4806: список гейта уходит в `node --test` ПАЧКАМИ, а не одной командой.
# Windows режет командную строку CreateProcess на 32767 символах, и на 693 файлах
# аргументы переросли лимит (34 КБ) — гейт падал `Argument list too long`, не выполнив
# НИ ОДНОГО теста, то есть на Windows его нельзя было прогнать вообще. Растёт корпус —
# растёт строка, так что дело было не в конкретном файле. Бюджет с запасом: сверх
# аргументов в строку входят путь к самому node и `--test`.
ARG_BUDGET=24000

# Прогнать переданные файлы пачками по длине аргументов. Возвращает 1, если упала
# ХОТЬ ОДНА пачка (иначе красный тест утонул бы за следующими зелёными), но гоняет
# их все — отчёт должен покрывать корпус целиком, а не обрываться на первой красной.
run_batched() {
  local rc=0 len=0 t
  local batch=()
  for t in "$@"; do
    if [[ ${#batch[@]} -gt 0 && $((len + ${#t} + 1)) -gt $ARG_BUDGET ]]; then
      node --test "${batch[@]}" || rc=1
      batch=(); len=0
    fi
    batch+=("$t"); len=$((len + ${#t} + 1))
  done
