(function(){
  ...
  function planLayouts(input){
    ...
    clusters.forEach(function(cluster){
      ...
      // #4757 (ТЗ §7/§15, решение заказчика 14.08.2026): КОНФИГУРАЦИЮ ЗАДАЁТ ЗАКАЗ.
      // Seed — «1 заказ = 1 резка» по ключу ЗАКАЗА: если под заказом указаны позиции с разными
      // ширинами, эти ширины и берутся, ОДНОЙ резкой. Перебор выгоды конфигурацию заказа не
      // разбирает — он вправе только доливать к ней чужое (и то не больше maxOrders заказов).
      //
      // Раньше ключ был (ЗАКАЗ, ШИРИНА), и ширины заказа собирались обратно «по выгоде» в refine.
      // По выгоде почти никогда и не собирались: замер на 400 случайных заказах — конфигурация
      // сохранялась в 21% случаев при 2 позициях, 3% при 3 и 0% при 4–5. Правило ТЗ §7
      // («разбивает позиции по заказам») не выполнялось.
      //
      // ЦЕНА, ПРИНЯТАЯ ЗАКАЗЧИКОМ. Выделенная резка даёт за проход 4–6 рулонов ОДНОЙ ширины,
      // комбинированная — по одному каждой, поэтому «одна резка на заказ» стои́т прогонов
      // (+4.6 / +9.8 / +17.0 при 3 / 4 / 5 позициях; резок при этом меньше на 1.5 / 2.1 / 2.7).
      // Целостность заказа важнее — это осознанный размен, а не побочный эффект.
      //
      // РАЗНЫЕ ЗАКАЗЫ по-прежнему не склеиваются принудительно (#3684: перебор по ширине джамбо →
      // лишний прогон и почти пустая резка-сирота). Позиция без orderId — своей группой.
      //
      // ОТКАТ, ЧТОБЫ НЕ ТЕРЯТЬ РАБОТУ. Одноширинный seed ГАРАНТИРОВАЛ, что composeLayout ничего
      // не сбросит в `overflow`: одна ширина в джамбо влезает всегда. Многоширинный такой
      // гарантии не даёт, а `overflow` эмитится в `skipped` с причиной «шире джамбо» — значит
      // позиции, не влезшие ВМЕСТЕ, выпали бы из плана, да ещё с ложной причиной. Поэтому заказ,
      // который целиком не укладывается (или у которого ширин больше maxWidths, #4755),
      // РАЗБИВАЕТСЯ по ширинам — как было до этой правки. Настоящее «шире джамбо» при этом
      // сохраняется: такая ширина отвергается и в разбиении.
      var seedOrder = [], bySeed = {};
      cluster.forEach(function(p){
        var owner = (p.orderId != null && String(p.orderId) !== '') ? ('o' + p.orderId) : ('p' + p.id);
        if (!bySeed[owner]) { bySeed[owner] = []; seedOrder.push(owner); }
        bySeed[owner].push({ width: p.width, qty: p.qty, positionId: p.id, stockable: p.stockable });
      });
      var groups = [];
      seedOrder.forEach(function(k){
        var demands = bySeed[k];
        var whole = buildGroup(demands);
        var widthCount = Object.keys(whole.demandByWidth).length;
        var wholeFits = !whole.overflow.length && (maxWidths <= 0 || widthCount <= maxWidths);
        if (wholeFits) { groups.push(whole); return; }
        // Заказ целиком не встаёт — раскладываем его по ширинам (прежнее поведение).
        var byWidth = {}, widthOrder = [];
        demands.forEach(function(d){
          var wk = 'w' + round3(d.width);
          if (!byWidth[wk]) { byWidth[wk] = []; widthOrder.push(wk); }
          byWidth[wk].push(d);
        });
        widthOrder.forEach(function(wk){ groups.push(buildGroup(byWidth[wk])); });
      });
