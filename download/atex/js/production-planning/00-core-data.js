...
(function(root, factory) {
    ...
    // #4833: В СМЕНЕ ЛИ СТАНОК СЕЙЧАС — { id: штамп последнего «Конца смены» } для станков,
    // чьё ПОСЛЕДНЕЕ событие (по всему пришедшему журналу, без отсечки по дню) — «Конец смены».
    // Та же мерка, что у пульта слиттера (#4332 п.2: открытость определяется последним
    // событием «Начало/Конец смены» этого станка, в каком дне оно записано — неважно).
    // Отличие от `shiftClosedSlitters`: там день ОБЯЗАН быть сегодняшним (это «день кончился»,
    // #4596), здесь же вопрос «работа прямо сейчас идёт?» — закрыл станок смену вчера вечером
    // и сегодня ещё не открывал, значит резки не было и просроченное начатое с неизвестным
    // фактом можно двигать вслепую. Журнал планирование читает за «сегодня + вчера» (#4371:
    // целиком нельзя — старые строки вытесняют свежие за лимитом), поэтому станок, простоявший
    // закрытым дольше окна, в карту НЕ попадает — консервативно, «не двигаем». Нет ни одного
    // события станка — его в карте тоже нет: неизвестно, идёт работа или нет.
    // Чистая — покрыта тестом.
    function shiftNotOpenSlitters(events, opts) {
        var o = opts || {};
        var byLabel = o.slitterIdByLabel || {};
        var last = {};
        (events || []).forEach(function(ev, i) {
    ...
    //     НАЧАТОЕ с НЕИЗВЕСТНЫМ фактом проходов (#4830) здесь НЕ исключение: смену закрыли —
    //     работа не идёт, двигаем целиком, считая факт нулём (разделить по неизвестному факту
    //     нельзя, а оставлять задание в закончившемся дне нельзя тем более);
    ...
    //     НАЧАТОЕ с НЕИЗВЕСТНЫМ фактом при ИДУЩЕЙ смене станка не двигается (#4381 — работа
    //     может идти); при ЗАКРЫТОЙ — едет ЦЕЛИКОМ, факт 0, как в группе «Смена закрыта»
    //     (#4833 — продолжение #4830);
    ...
    function deviationSettlePlan(cuts, groups, opts) {
        ...
        // #4833: станки, НЕ В СМЕНЕ прямо сейчас (последнее событие журнала — «Конец смены»,
        ...
        [].concat(pending, g.earlyRun || []).forEach(function(c) {
            ...
            var done = cutDoneRuns(c);
            // #4885: задание НАЧАТО, резок 0 — настройка выполнена в день раньше план-дня
            // (оператор наладил станок под завтрашнюю резку). Настройка переезжает записью
            // с 0 резок в день выполнения (обычный setup-сегмент #3635); записи связываются
            // «ID выполненной части» (#4651). Гейт — ВЕСЬ pending (overdue ∪ shiftClosed):
            // урегулировать могут как вечером план-дня (смена закрыта), так и на следующий
            // день, когда задание уже просрочено, — исход дефекта не зависит от того, в какой
        ...
        pending.forEach(function(c) {
            ...
            // #4830/#4833: у станка, который СЕЙЧАС НЕ В СМЕНЕ, это рассуждение не работает —
            // на нём ничего НЕ ИДЁТ (типичный случай: настройка выполнена — «Начато» стоит, а
            // резок до закрытия смены не было). Двигаем ЦЕЛИКОМ, считая факт нулём; делить по
            // неизвестному факту всё равно нечем, а оставлять работу в дне, которого у станка
            // больше нет, — возвращение бага #4596. Это группа «Смена закрыта» (#4830) и
            // просроченные у станка вне смены (#4833).
            if (!sp && cutIsStarted(c) && cutDoneRuns(c) == null
                    && !shiftClosedSet[String(c.id)]
                    && notOpenSlitters[cutSlitterKey(c)] == null) return;
            var sid = cutSlitterKey(c);
