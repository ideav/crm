...
(function(root, factory) {
    ...
    function deviationSettlePlan(cuts, groups, opts) {
        ...
        pending.forEach(function(c) {
            ...
            // #4830: у «Смена закрыта» это рассуждение не работает — смену станок закрыл,
            // работа на нём НЕ ИДЁТ (типичный случай: настройка выполнена — «Начато» стоит,
            // а резок до закрытия не было). Двигаем ЦЕЛИКОМ, считая факт нулём; делить по
            // неизвестному факту всё равно нечем, а оставлять работу в дне, которого у
            // станка больше нет, — возвращение бага #4596.
            if (!sp && cutIsStarted(c) && cutDoneRuns(c) == null
                    && !shiftClosedSet[String(c.id)]) return;
            var sid = cutSlitterKey(c);
