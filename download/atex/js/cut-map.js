...
// `object/{Задание в производство}/?JSON_OBJ[&F_I=…]`; состав резки (полосы) — её
// подчинённые записи `object/{Партия ГП}/?F_U={резкаId}` (#3242: состав резки теперь
// «Партия ГП», а не «Полоса»). Ширина входа
...
(function(root, factory) {
    ...
    var TABLE = {
        ...
        // #3242: состав резки теперь «Партия ГП» (подчинённая), а не «Полоса».
        strip: 'Партия ГП',
    ...
    // #3431/#3433: геометрия раскроя (число ножей = полос за проход) — «Кол-во полос».
    // НЕ «Кол-во рулонов»: с #3433 там спрос (рулоны под заказ), а с #3431 геометрия
    // переехала в «Кол-во полос». «Назначение» в «Партии ГП» нет → purpose best-effort.
    ...
    AtexCutMap.prototype.loadCut = function(id) {
        ...
        return this.getJson('object/' + cutMeta.id + '/?JSON_OBJ&F_I=' + encodeURIComponent(id)).then(function(rows) {
            var rec = (rows || [])[0];
            if (!rec) throw new Error('Задание в производство #' + id + ' не найдено');
