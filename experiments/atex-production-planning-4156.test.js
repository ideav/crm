...
// планировщика): рост числа полос стоил KNIVES_INCREASE_COST_MN=50 вместо плоских 30 (#3600). Отчёт
// «Комбинации по всем позициям» суммирует ХРАНИМЫЕ колонки наладки задания (setupActivityColumns →
// firstSetupParts/changeoverParts): ножи плоско 30. Отсюда суммы наладки расходились при совпадении
// числа заданий (114).
// #4296: первой резке ПУСТОГО станка теперь засчитывается заправка сырья (firstSetupParts даёт
// KNIFE + MATERIAL_WINDING) — и в хранимых колонках, и в planQuality/идеале. Тест обновлён под это.
//
...
(function () {
    ...
    // #4296: первая резка ПУСТОГО станка ставит ножи 30 + заправку сырья 15 (сырьё на станок с нуля).
    assertEqual(cols['A'], { knifeMin: 30, materialWindingMin: 15 }, '#4296 канон: первая резка пустого станка — ножи 30 + заправка сырья 15');
