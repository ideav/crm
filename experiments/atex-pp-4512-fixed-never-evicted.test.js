...
// ── 6. ЛЕСТНИЦА #4467 НЕ ТРОНУТА (пока так, решение заказчика 30.07.2026) ─────────────────
// День переполнен САМИМИ 🔒 (три по 200 мин = 690 при потолке 460) — лестница по-прежнему
// увозит лишние. Если это поведение поменяется, тест обязан упасть и потребовать решения.
(function () {
    var segs = pack([cut('F1', 'MA', [30, 29], 1, 200, 0),
                     cut('F2', 'MB', [60, 15], 1, 200, 0),
                     cut('F3', 'MC', [150, 7], 1, 200, 0)]);
    var load = loadByDay(segs);
    assert(Object.keys(load).length > 1, 'ПОКА ТАК: реальное переполнение дня всё ещё увозит 🔒',
        '(' + JSON.stringify(load) + ')');
