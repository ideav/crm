...
//   E — и на путях ПЛАНА (Сгенерировать / пересчёт / ручной перенос) фолбэк не срабатывает НИ РАЗУ.
//
...
// ── E: на пути ПЛАНА фолбэк не срабатывает НИ РАЗУ ──────────────────────────────────────────
// Главная гарантия тикета: гадание невозможно не потому, что «мы его починили», а потому что на
// путях плана оценка не вызывается вовсе. Ловим это прямо: полный прогон генерации не должен
// напечатать ни одного предупреждения о переходе на оценку.
(function () {
    var warned = [];
    var origWarn = console.warn;
    console.warn = function () { warned.push(Array.prototype.slice.call(arguments).join(' ')); };
    try {
        plan(corpus(), PP);                                         // «Сгенерировать»
        plan(corpus(), PP, { preserveOrder: true });                 // «Пересчитать наладку» / ручной порядок
        var lock = { X: 1 };
        plan(corpus(), PP, { dayLockByCut: lock, manualMoveByCut: { X: 1 },
                             wholeDayCutIds: ['X'], pinCutIds: ['X'] });   // ручной перенос 🗓
    } finally { console.warn = origWarn; }
    var fell = warned.filter(function (m) { return /#4547/.test(m); });
    assert(fell.length === 0,
        '#4547-E: ни один путь плана не переходит на оценку — день везде от упаковщика',
        '(' + (fell[0] || 'предупреждений нет') + ')');
})();
