...
//    B(seq2): наладка 45 (ножи 30 + сырьё 15) + 5 проходов × 10 = 95. В день 2 после продолжения
//    A (50 мин) влезает наладка, но ни одного прохода — по #3635 п.5 наладка кладётся ХВОСТОМ
//    в конец дня 2 (сегмент 0 проходов), а проходы уходят продолжением на день 3. Так день
//    заполняется до конца — ровно то, ради чего заведён #3619.
...
assertEqual(split.updates.map(function(u) { return { id: u.cutId, seq: u.sequence, runs: u.plannedRuns }; }),
    [ { id: 'A', seq: 1, runs: 10 }, { id: 'B', seq: 3, runs: 0 } ],
...
assertEqual(split.creates.map(function(c) { return { parent: c.parentCutId, seq: c.sequence, runs: c.plannedRuns }; }),
    [ { parent: 'A', seq: 2, runs: 5 }, { parent: 'B', seq: 4, runs: 5 } ],
    '#3619/#3635 п.5: остаток A → продолжение (seq2, 5 проходов); наладка B — хвостом в конец дня, '
    + 'её проходы — продолжением (seq4)');
...
// раскладываются обратно в A(seq1,10)+Acont(seq2,5): запись продолжения ПЕРЕИСПОЛЬЗОВАНА, ни одной
// новой для A. B разбивается впервые (наладка хвостом, проходы продолжением, #3635 п.5) — его
// продолжения в очереди ещё нет, поэтому одна запись создаётся.
assertEqual(again.creates.map(function(c) { return c.parentCutId; }), ['B'],
    '#3619/#3427: повторная раскладка НЕ создаёт продолжений уже разбитой цепочки A (переиспользует запись)');
