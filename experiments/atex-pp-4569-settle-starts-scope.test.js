...
//     в набор пересчёта помимо диапазона. Замороженный день в ПЕРЕСЧЁТ СТАРТОВ не входит: он идёт
...
// ── 8. #4577: дни ручного действия для него РАЗМОРОЖЕНЫ ─────────────────────
// «Ручная перестановка игнорирует замороженность дня» (правило заказчика). Заморозка живёт в
// ЧЕТЫРЁХ механизмах — пиннинг входа, предикат упаковщика, страж записи, выравнивание дня — и
// снимать её надо во всех сразу. Иначе получается полумера: работу в замороженный день положили
// (закрепив её, #4574), а вынести лишнее оттуда некому — боевое #4577: 95 мин остатка + 425 мин
// 🔒 = 520 при потолке 455, и выравнивание этого переполнения даже НЕ ВИДЕЛО.
(function() {
    var planning = require('../download/atex/js/production-planning.js').planning;
    var cuts = [{ id: 'rest', slitter: { id: '1' }, planDate: String(tsAt(2026, 8, 3, 8, 0)) }];
    var scope = planning.settleMoveScope([], ['rest'], cuts, [20260803]);
    assertEqual(scope.unfrozenDayKeys, [20260803],
        '#4577 действие называет дни, которые для него разморожены');

    // Пересчёт стартов: ручной вызов заморозку игнорирует, автоматический — нет.
    var c = makeController();
    assertEqual(c.recalcScopeCutIds('1', { dayKeys: [20260803] }).indexOf('frozen'), -1,
        'автоматический пересчёт замороженный день по-прежнему не трогает');
    assertEqual(c.recalcScopeCutIds('1', { dayKeys: [20260803], manual: true }).indexOf('frozen') >= 0, true,
        '#4577 РУЧНОЙ пересчёт видит задания замороженного дня — иначе переполнение невидимо');
})();

// ── 9. #4582: дни действия входят и в ОБЛАСТЬ ВИДИМОСТИ выравнивания ─────────
// #4577 разморозил дни для ручного действия, но состав дня выравнивание берёт через
// recalcScopeCutIds, а он ограничен окном фильтра [С;По]. Оператор жмёт кнопку на одном дне, а
// работа уезжает в другой — переполнение там оставалось НЕВИДИМЫМ (боевое #4582: Станок 3,
// 03.08 = 621 мин при потолке 455, все задания 🔒, и ни строки о выравнивании в логе).
(function() {
    var c = makeController();
    // Фильтр показывает только 02.08. Ограничений ДВА и они разные: заморозка (03.08) и окно
    // [С;По] (04.08). `manual` снимает первое, `dayKeys` — второе; нужны ОБА.
    assertEqual(c.recalcScopeCutIds('1', { manual: true }).sort(), ['visible'],
        'один manual окно фильтра НЕ расширяет — 03.08 и 04.08 остаются за ним');
    assertEqual(c.recalcScopeCutIds('1', { dayKeys: [20260803] }).sort(), ['visible'],
        'одни dayKeys заморозку НЕ снимают — 03.08 всё ещё не виден');
    assertEqual(c.recalcScopeCutIds('1', { manual: true, dayKeys: [20260803, 20260804] }).sort(),
        ['faraway', 'frozen', 'visible'],
        '#4582 вместе: виден и ЗАМОРОЖЕННЫЙ день, и тот, что за окном фильтра — иначе переполнение невидимо');
})();
