// UI test for ideav/crm#4052 (ревизия #4110) — обед и перерывы на Ганте: серые накладки ПОВЕРХ
// баров, а не отдельные строки. #4110: несущий их бар РАСШИРЯЕТСЯ на длительность накладки, а сама
// накладка кладётся НА бар (а не «висит в конце», как после #4099). Накладка без текста, с
// title «Обед 12:20-13:00» / «Перерыв 10:00-10:10»; в легенде — единый серый пункт «Обед / перерыв».
//
...
// #4110: бар 3797 УДЛИНЯЕТСЯ на обед (работа 65 + обед 40 = 105 мин), а накладка обеда лежит
// НА баре — не «висит в конце» (ppm=2 в стабе, поэтому carrW = минуты × 2).
var bars = body.querySelectorAll('.atex-cg-bar');
var carrier = bars[1];   // порядок строк = 3784, 3797, 3790
var carrLeft = parseFloat(carrier.style.left), carrW = parseFloat(carrier.style.width);
assert(Math.abs(carrW / 2 - 105) < 1, '#4110: бар-обеда удлинён на обед (65 работа + 40 обед = 105 мин)');
var band = bands[0];
var bandLeft = parseFloat(band.style.left), bandW = parseFloat(band.style.width);
assert(bandLeft >= carrLeft - 0.5 && (bandLeft + bandW) <= (carrLeft + carrW) + 0.5,
    '#4110: накладка обеда целиком лежит НА своём баре (не пририсована в конце)');
