// UI test for ideav/crm#4052 (ревизия #4099) — обед и перерывы на Ганте: серые накладки ПОВЕРХ
// баров, а не отдельные строки. #4099 «рисуй как есть»: несущий бар БОЛЬШЕ НЕ удлиняется на обед/
// перерыв — накладка лежит поверх реального бара по реальному времени; накладка без текста, с
...
// #4099: бар 3797 — РЕАЛЬНОЙ ширины (65 мин), НЕ растянут на обед; накладка обеда лежит поверх
// него, а до следующего бара остаётся реальный зазор («рисуй как есть»).
...
assert((nextLeft - (carrLeft + carrW)) > 1, '#4099: между баром-обедом и следующим есть реальный зазор (бар не растянут)');
assert(Math.abs(carrW / 2 - 65) < 1, '#4099: бар-обеда = реальная работа 65 мин (не удлиняется на обед)');
