...
(function() {
    // (#4902: завершение требует накопленный погонаж; #4914: номер джамбо обязателен
    // и живёт в записи «Номера джамбо», не в реквизите резки)
...
(function() {
    ...
    // #4914 (решение заказчика): «Брак, м» — реквизит записи «Номера джамбо», из
    // показаний резки выведен (очистка при отметке отменена) — в теле его нет вовсе;
    // «Примечания» остаются и пустым значением доезжают.
    assert(!('t8458' in f), '#4914: «Брак, м» из показаний резки выведен — реквизит не пишется');
    assert(f.t1171 === '', '#4366: очищенные «Примечания» уходят пустыми');
})();
