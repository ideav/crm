...
(function () {
    ...
    // находит вовсе.
    //
    // ОЖИДАНИЕ ИЗМЕНЕНО (#4467, решение заказчика): раньше здесь утверждалось «переполнение дня 🔒
    // не выталкивает» — и день разбухал за смену. Теперь ПОТОЛОК ДНЯ СИЛЬНЕЕ ЗАМКА ДНЯ: fix2
    // уезжает вперёд, пока не найдёт день с местом (день 1 занят продолжением bigfix → день 2).
    // Замок дня остаётся абсолютным ровно там, где место есть, — это проверяет тест
    // experiments/atex-pp-4467-day-ceiling.test.js (контроль «место есть — 🔒 держит свой день»).
    ...
    assertEqual(res.days['fix2'].day, 2, 'п.1 (#4467): 🔒 fix2 вытеснен вперёд — в дне 0 и 1 места нет');
