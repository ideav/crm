...
// ── п.1: 🔒 держит свой день, пока в дне есть МЕСТО (#4467 уточнил переполнение) ────────────────
(function () {
    // bigfix (300 проходов, 🔒 на дне 0) не влезает в день и рвётся по потолку (#4304 — это про
    // размер самой резки, голова остаётся на дне 0). fix2 (🔒 на том же дне 0) места в дне не
    // находит вовсе.
    //
    // ОЖИДАНИЕ ИЗМЕНЕНО ДВАЖДЫ — и это история одного правила, а не путаница.
    //   • Изначально: «переполнение дня 🔒 не выталкивает» — день разбухал за смену.
    //   • #4467 (решение 28.07.2026): ПОТОЛОК ДНЯ СИЛЬНЕЕ ЗАМКА — fix2 уезжал вперёд (день 2).
    //   • #4511/#4512/#4513 (решение 30.07.2026): ОТМЕНЕНО обратно — «не вытеснять зафиксированные
    //     из дня НИ ПРИ КАКИХ ОБСТОЯТЕЛЬСТВАХ. Можно разорвать последнее задание». Поэтому fix2
    //     ОСТАЁТСЯ на своём дне 0 (при необходимости разрывается по потолку, #4304), а день законно
    //     уходит за потолок — приёмка #4512 это следствие называет прямо.
    // Правило для НЕЗАФИКСИРОВАННЫХ не менялось: потолок их по-прежнему вытесняет.
    ...
    assertEqual(res.days['fix2'].day, 0, 'п.1 (#4512): 🔒 fix2 НЕ вытеснен — остался на своём дне 0');
