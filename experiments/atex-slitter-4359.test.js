...
// #4919: открытость смены — свойство СТАНКА, события любого оператора учитываются.
// (Раньше здесь проверяли обратное — «чужой оператор отсеивается», — но при per-пользовательской
// смене одно и то же событие на разных устройствах давало разное состояние.)
var alienUser = makePult([{ id: '7', when: '2026-07-24 12:00:00', type: 'Начало смены', userId: '999', slitterId: '1277' }]);
assert(alienUser.isShiftOpen() === true, '#4359/#4919: смена, открытая другим оператором на этом станке, открыта');
var alienClosed = makePult([{ id: '7', when: '2026-07-24 12:00:00', type: 'Конец смены', userId: '999', slitterId: '1277' }]);
assert(alienClosed.isShiftOpen() === false, '#4359/#4919: «Конец смены» другого оператора закрывает смену станка');

// отметки проходов задания будущего дня тоже не теряются (#4332 п.4 + #4348).
// #4564: проходы хранит само задание («Кол-во резок факт»), поэтому выбранный в тулбаре
// день на них не влияет ВОВСЕ — даже если события той смены не загружены.
var passes = makePult([]);
assertEqual(passes.donePassCount({ id: '625873', actualRuns: '2' }), 2,
    '#4359/#4564: отмеченные проходы считаются независимо от выбранного дня (было 0 → «Резка 1 из M» врала)');
