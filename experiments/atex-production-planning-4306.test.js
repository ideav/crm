...
// ── #4392: зафиксированные — НЕ «стены»: перенос ЧЕРЕЗ фикс и самого фикса РАЗРЕШЁН ─────────────────
(function () {
    var d = [
        { id: 'A', planDate: 1000 },
        { id: 'F', planDate: 2000, fixed: true },   // раньше «стена» (#3508/#4306), теперь переставляется
        { id: 'C', planDate: 3000 },
        { id: 'D', planDate: 4000 }
    ];
    // Перетащить D (за фиксом) на A (до фикса) — проходим ЧЕРЕЗ F. #4392: больше НЕ запрет.
    var thru = planDragReorder(d, 'D', 'A');
    assertEqual(thru.error, null, '#4392: перенос ЧЕРЕЗ зафиксированное — БЕЗ ошибки (фикс не «стена»)');
    var byId = {}; thru.assignments.forEach(function (w) { byId[w.id] = w.planStartTs; });
    assertEqual(byId.F, 3000, '#4392: зафиксированный F переставлен вместе с остальными (2000→3000)');
    assertEqual([byId.D, byId.A, byId.F, byId.C], [1000, 2000, 3000, 4000],
        '#4392: времена дня переназначены по новому порядку [D,A,F,C], включая фикс');
    // Перетащить САМ зафиксированный F — тоже разрешено (меняет позицию в пределах дня).
    assertEqual(planDragReorder(d, 'F', 'D').error, null, '#4392: сам зафиксированный можно перетащить');
})();
