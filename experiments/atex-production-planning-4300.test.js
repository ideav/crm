...
//
// Фикс #4300: восстанавливаем заправку станка = конфигурация ПОСЛЕДНЕЙ (по planStart) резки прошлых
// дней; ею переопределяем prevSetupBySlitter. Первая резка окна той же конфигурации получает 0
// наладки — упаковщик и хранимые колонки сходятся, дыры нет.
//
// #4312: помощник prevSetupFromExcludedCuts(cuts, keepIds) заменён на prevSetupBeforeWindow(cuts, base) —
// заправку берём по ВСЕЙ очереди станка (любой статус/замок), а не только по резкам, вырезанным из
// planInput механизмом #4294: «Завершён» и 🔒-цепочки мимо прежнего источника проходили, и дыра
// возвращалась. Сценарии ниже — те же, источник заправки новый.
//
...
var BASE_MS = new Date(2026, 6, 22, 0, 0, 0, 0).getTime();   // «С» = 22.07 = день 1 шкалы cut() ниже

// Конфигурация резок станка m1. planDate — unix-сек (DATETIME, старт задания). База «С» = день 1
// (22.07); день 0 = 21.07, раньше «С». A — вчерашняя резка (исключается #4294). B — первое задание
// окна той же конфигурации (MW308, те же ножи). C — следующее задание (MR194, другие ножи+сырьё).
function cut(id, dayOffset, mat, kw, runs) {
    var ms = BASE_MS + (dayOffset - 1) * 86400000 + 8 * 3600000;
    return { id: id, slitter: { id: 'm1' }, materialId: mat, winding: 'OUT', batchId: '',
        ...
        planDate: String(Math.floor(ms / 1000)), duration: String(runs * 30) };
}
...
// ── 1) prevSetupBeforeWindow: заправка = конфигурация последней резки станка раньше «С» ─────────────
(function () {
    var carry = planning.prevSetupBeforeWindow([A, B, C], BASE_MS);
    // #4314: dayOffset — день, который заправка описывает (нужен правилу сброса наладки после отпуска).
    eqo(carry.m1, { materialId: 'MW308', winding: 'OUT', knifeWidths: [59, 59], knifeCount: 2, dayOffset: -1 },
        '#4300: заправка m1 восстановлена из вчерашней резки A (MW308, ножи 59,59)');
    // Несколько прошлых дней — берём ПОСЛЕДНЮЮ (макс. planStart).
    var A0 = cut('A0', -2, 'MR300', [70], 1), A1 = cut('A1', 0, 'MW308', [59, 59], 1);
    eqo(planning.prevSetupBeforeWindow([A0, A1, B], BASE_MS).m1.materialId, 'MW308',
        '#4300: при нескольких прошлых днях берём позднейшую (MW308, не MR300)');
    // Нет резок раньше «С» → никого не трогаем.
    eqo(planning.prevSetupBeforeWindow([B, C], BASE_MS), {}, '#4300: нет резок раньше «С» → {}');
})();
...
(function () {
    ...
    var carryY = planning.prevSetupBeforeWindow([Ay, By], BASE_MS);
    var optsY = {
        dayStartMin: 0, dayEndMin: 600, times: TIMES,
