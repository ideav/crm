...
// ── 1. Напихали 🔒 больше ёмкости — день ЗАКОННО уходит за потолок ───────────────────────
// Три 🔒 по 200 мин работы на день 0: 3 × (30 наладки + 200) = 690 при потолке 460.
//
// ОЖИДАНИЕ ИЗМЕНЕНО (#4511/#4512/#4513, решение заказчика 30.07.2026). Здесь утверждалось «день не
// разбухает, лишнее уехало» — это правило #4467 («потолок дня сильнее замка дня», 28.07.2026).
// Заказчик его ОТМЕНИЛ обратным: «не вытеснять зафиксированные из дня НИ ПРИ КАКИХ
// ОБСТОЯТЕЛЬСТВАХ», и подтвердил следствие в приёмке #4512: день, набитый одними 🔒, ОСТАНЕТСЯ за
// потолком — это видимый факт (бейдж минут), а не повод сдвинуть замок. Цена прежнего правила —
// #4513: зафиксированный «паровоз» уехал из 30.07 в 31.07 ради нескольких свободных заданий.
// Потолок для НЕЗАФИКСИРОВАННЫХ заданий действует как прежде — это проверяют случаи 3 и 4 ниже.
(function () {
    var segs = pack([cut('F1', 'MA', [30, 29], 1, 200, 0),
                     cut('F2', 'MB', [60, 15], 1, 200, 0),
                     cut('F3', 'MC', [150, 7], 1, 200, 0)]);
    var load = loadByDay(segs);
    assert(Object.keys(load).length === 1 && load[0] > CEILING,
        'все 🔒 остались в дне 0, и он законно за потолком (#4512)',
        '(' + JSON.stringify(load) + ' при потолке ' + CEILING + ')');
})();
...
// ── 5. В остаток дня не влезает НИ ОДНОГО прохода — 🔒 ВСЁ РАВНО остаётся ───────────────
// F1🔒 съедает день (445 мин), F2🔒 — ОДИН проход 120 мин: 45 наладки + 120 в остаток не влезут.
//
// ОЖИДАНИЕ ИЗМЕНЕНО (#4512, решение заказчика 30.07.2026) — это ровно тот случай, который приёмка
// тикета назвала прямо: «день, набитый одними 🔒, которые уже нельзя разорвать (у каждого остался
// один проход — ПРОХОД НЕДЕЛИМ), останется за потолком». Разорвать F2 нечем: проход один. Значит
// уехать она не может (вытеснение 🔒 отменено) и разбиться не может (делить нечего) — остаётся на
// своём дне, а день уходит за потолок.
(function () {
    ...
    assert(Number(fSegs[0].dayOffset) === 0, '🔒 с единственным проходом осталась в своём дне (#4512)',
        '(день ' + fSegs[0].dayOffset + ')');
    assert((loadByDay(segs)[0] || 0) > CEILING, 'и день 0 законно за потолком',
