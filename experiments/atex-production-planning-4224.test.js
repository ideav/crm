...
// ── 1) ЗАФИКСИРОВАННУЮ просроченную фольгу НЕ ДВИГАЕМ (#4434 п.1 отменил рескью 🔒) ─────────────────
// База пн 06.07 (будни). m1 ручной порядок P,Q,X; X — ЗАФИКСИРОВАННАЯ фольга, срок d0. 1 резка/день ⇒
// P→d0, Q→d1, X→d2 (просрочка d2>d0). #4224 требовал вернуть X в срок; #4434 п.1 это ОТМЕНИЛ: замок
// абсолютен — 🔒 не переезжает ни ради срока, ни по какой другой причине. Просрочка не прячется:
// задание остаётся в ops.overdue (панель «просрочено» + лог #4200), решение за оператором.
(function () {
    ...
    assert(overdueIds(ops).indexOf('X') >= 0,
        '#4434 п.1: ЗАФИКСИРОВАННАЯ просроченная X НЕ рескьюится, а честно попадает в ops.overdue');
    assert(dayOf(ops, base, 'X') === 2,
        '#4434 п.1: X (🔒) осталась на своём месте очереди — день ' + dayOf(ops, base, 'X') + ' (не тянем в срок)');
    // Станок в обновлении появляется, лишь если он СМЕНИЛСЯ: пусто = «остался на m1».
    assert(machineOf(ops, 'X') !== 'm2',
        '#4434 п.1: X (🔒) не мигрировала на пустой станок m2 (станок в обновлении: ' + machineOf(ops, 'X') + ')');
})();
