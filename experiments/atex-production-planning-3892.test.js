...
// ── 1c: БЕЗ firstPartId цепочки нет — какими бы похожими записи ни были (решение заказчика
//     31.07.2026: запасное правило «конфигурация + смежные дни + заказ» убрано, оно гадало).
//     Маркер проставлен всем боевым записям (ateh 31.07.2026: 158 из 158).
(function () {
    var a = ch('A', '', 0, 7, 'ORD'); var b = ch('Acont', '', 1, 3, 'ORD');
    var m = mergeContinuationChains([ a, b ]);
    assertEqual(m.chainByLogical, { A: ['A'], Acont: ['Acont'] },
        '1c: без маркера — две самостоятельные записи, а не цепочка');
    assertEqual(m.cuts.length, 2, '1c: обе остались логическими резками');
    assertEqual(m.deletes, [], '1c: ничего не удаляется');
})();

// ── 1e (#3892, корень defect B): записи одной конфигурации в соседние дни НЕ сливаются — ни при
//     разных заказах, ни при пустых. Раньше при пустом заказе эвристика их всё-таки склеивала
//     (#3808), и голова уезжала на ранний день вне scope #3660 — «Упорядочить» пропускал резку.
(function () {
    ...
    assertEqual(me.chainByLogical, { E1: ['E1'], E2: ['E2'] }, '1e: и при пустом заказе тоже — цепочку делает только маркер');
})();
...
// ── 3 (defect B, корень): резки ОДНОЙ конфигурации, но РАЗНЫХ заказов (orderId), НЕ склеиваются
//     в одну цепочку (#3892) — «Упорядочить» их не пропускает. #3974: scope-фильтр входа отменён,
//     поэтому обе резки планируются от «С»; чужая ранняя резка (02.07) стягивается к «С», а не
//     остаётся вне окна. scopeFromKey/scopeToKey передаём, чтобы показать: они игнорируются.
(function () {
    ...
    assert(ids.indexOf('today') >= 0, '3(B): резка дня планируется (не пропущена из-за чужой ранней резки)');
    assert(ids.indexOf('prev') >= 0, '3(B) #3974: чужая ранняя резка (02.07) тоже планируется — scope не фильтрует вход, стягивается к «С»');
})();
