...
// (весь план) и qualityAll не изменились (совместимость + всплывающая подсказка). Отрицательный
// «избыток» при НЕпустом окне сохраняется (это «план лучше идеала» за счёт заправки, см. 3989-p3).
//
...
// Поля всего плана (совместимость + подсказка) остаются ненулевыми — их панель НЕ показывает.
eq(off.ideal.count, 4, 'весь план (совместимость): идеал 4');
eq(off.combinations, 3, 'весь план (совместимость): 3 комбинации');

// ── 2. Частичное окно 03.07..03.07: идеал/комбинации ОКНА только за день; факт < идеал ────────
console.log('\n== #4013: частичное окно 03.07 — оконный идеал, отрицательный избыток НЕ занулён ==');
...
eq(d3.idealWindow.count, 3, 'окно 03.07: идеал ОКНА 3 (набор {100} + сырьё A,B) — БЕЗ {60,60} с 04.07');
eq(d3.combinationsWindow, 2, 'окно 03.07: 2 комбинации ОКНА ({100·A},{100·B}) — БЕЗ {60·B}');
eq(d3.qualityWindow.excessCount, 1 - 3, 'окно 03.07: избыток 1−3 = −2 (план лучше идеала, заправка) — НЕ занулён');
// Поля всего плана видят и 04.07 → шире окна.
eq(d3.combinations, 3, 'весь план: 3 комбинации (виден и {60·B} с 04.07)');
eq(d3.ideal.count, 4, 'весь план: идеал 4');
...
eq(full.idealWindow.count, 4, 'полное окно: идеал ОКНА 4 == весь план');
eq(full.combinationsWindow, 3, 'полное окно: 3 комбинации ОКНА == весь план');
eq(full.qualityWindow.excessCount, 2 - 4, 'полное окно: избыток 2−4 = −2 (заправка сэкономила старт)');
