...
(function () {
    ...
    // Стык дней: 🔒 лежит в дне 1 (день 0 добит). #4542 (решение заказчика 31.07.2026): точка перед
    // такой 🔒 закрыта и здесь — автоматика её не обгоняет. Прежде правило смотрело только ВНУТРЬ дня
    // замка, и задание без хранимого места вставало в день 0, обгоняя 🔒 дня 1.
    var full = [S('A', { wm: 400, ts: 0 }), S('L', { fixed: true, wm: 90, ts: 86400 })];
    assert(P.scorePosition(full, 1, S('X', { wm: 40 }), ctx) === null,
        'C4 на стыке дней точка перед 🔒 следующего дня тоже недопустима (#4542)');
    // А стоявшее перед этой 🔒 в хранимом плане своё место сохраняет — правило не переворачивает дни.
    var early = P.scorePosition(full, 1, S('X', { wm: 40, ts: 3600 }), ctx);
    assert(early !== null && early.dayOffset === 0,
        'C4b стоявшее перед 🔒 задание встаёт в день 0, как и раньше',
        '(' + (early ? 'день ' + early.dayOffset : 'null') + ')');
...
(function () {
    ...
    // #4542: H и Y стоят в ХРАНИМОМ плане ПЕРЕД 🔒 M (день 0 против дня 1) — запрет обгона замка
    // их не касается, и проверяется ровно то, ради чего написан блок: разрыв задания на стыке дней.
    var H = cut('H', 300, { day: 0, ts: 1000 });
    var Y = cut('Y', 50, { runs: 6, day: 0, ts: 2000 });
