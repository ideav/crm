...
// ── 2) Слот-режим: фольга — штрафом, а НЕ жёстко в конец (может стоять раньше, если дешевле) ──────
// Один станок, вход [F(фольга,M1), A(M1)]. Обе одного сырья: перестановка не экономит переналадку,
// поэтому фольга остаётся там, где во входе (штраф FOIL_NOTEND применится лишь если это удешевит —
// здесь нет). Проверяем, что слой НЕ форсирует жёсткий «фольга-последней» (в отличие от orderCuts).
(function () {
    ...
    assert(JSON.stringify(orderOn(ops, 'm1')) === JSON.stringify(['F', 'A']),
        '#4085 wiring: фольга не форсируется в конец жёстко (порядок F,A сохранён — фольга штрафом, не правилом)');
})();
