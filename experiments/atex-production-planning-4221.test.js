...
(function () {
    ...
    // #4487: 🔒 держит ДЕНЬ, а не голову дня — позицию в дне выбирают веса и с фиксацией тоже.
    // Прежде эта же строка утверждала обратное («🔒-прикол сажает M в индекс 0 — не по весу»), и
    // ровно на это пожаловался заказчик: задание, перенесённое «По весу» с галкой «Зафиксировать»,
    ...
    assert(Math.abs(iMp - iQp) === 1,
        '#4487: с галкой «Зафиксировать» M тоже встаёт рядом с одноимённым Q (порядок ' + idsPin.join(',') + ')');
    assert(sp.every(function (o) { return o.day === 0; }),
        '#4487: и день при этом остаётся выбранным — фиксация держит ДЕНЬ (порядок ' + idsPin.join(',') + ')');
})();
