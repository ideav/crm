...
// CHAIN_SETUP_ONCE · SUPPLY_CONSERVED · DAY_CAPACITY (#4759). Законные исключения берутся у самого
// движка (`ops.fixedDayLost`, `ops.fixedDayHeld`), а не пересчитываются здесь.
//
// ЧТО НЕ ГЕЙТИТ И ПОЧЕМУ — у каждого правила названа причина в месте объявления:
// ORDER_PARTS_ADJACENT держит хвост контроллера, DAY_FILL — мерка, а не запрет.
//
...
function ruleCtx(c, ops) {
    ...
    // #4759 п.2: станко-дни с ЗАКОННЫМ перебором — вердикт раскладки (#4512). Станок берём ИЗ
    // ВЕРДИКТА: на входах со слоем размещения задание переназначают на другой станок (#4001), и
    // станок исходной записи указал бы на чужой день. Ключ — как у занятости и потолка.
    var held = {};
    ((ops && ops.fixedDayHeld) || []).forEach(function(h) {
        held[String(h.slitterId) + '|' + dayKeyOfOffset(Number(h.fixedDay))] = true;
    });
    return {
...
// #4759: DAY_CAPACITY ЗДЕСЬ С 18.08.2026. Первый прогон этого фаззера нашёл по нему 70 срабатываний
// и ДВЕ разные причины, обе — расхождение мерок, а не «плохой план»:
//   • потолок дня выбирает ЕГО ХВОСТ (решение заказчика 17.08.2026): наладка на конце → 450+TUNE,
//     резка → 450+CUTS. Судили все дни потолком резки, и законный день, кончающийся наладкой,
//     объявлялся нарушением на 1…5 минут (п.1, 70 → 8);
//   • перебор РАДИ 🔒: замок вытеснять нельзя (#4512), голова остаётся в дне в один проход, и день
//     законно длиннее смены. Раскладка этот вердикт не объявляла — теперь объявляет сама
//     (`ops.fixedDayHeld`, п.2, 8 → 0).
// Мерка выданной индульгенции на корпусе 300×5: станко-дней 5992, за потолком 14, и ровно эти 14
// объявлены законными — ни одного дня «на всякий случай». Поэтому правило ГЕЙТИТ.
var HARD = ['FROZEN_DAY', 'FIXED_CUT_DAY', 'CHAIN_CONTIGUOUS',
            'CHAIN_SETUP_ONCE', 'SUPPLY_CONSERVED', 'DAY_CAPACITY'];
