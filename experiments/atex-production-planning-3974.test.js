...
// #4542 (решение заказчика 31.07.2026): «Дата план» свободное задание по-прежнему НЕ держит, но и
// 🔒 оно не обгоняет. Nlate хранится на дне 3 — там же, где Ffix, — поэтому к «С» его больше не
// тянут: раньше замка автоматика не ставит ничего.
assertEqual(r4.day.Nlate >= 3, true,
    '#4542: свободное задание, хранящееся не раньше 🔒, замок не обгоняет');
// Без замка на пути — прежнее поведение #3974: день не держится, задание стягивается к «С».
var r4b = plan([
    cut('N1', 'matA', 2, 0), cut('N2', 'matA', 2, 0), cut('Nlate', 'matA', 2, 3)
]);
assertEqual(r4b.day.Nlate < 3, true,
    '#3974: свободное задание с «Датой план» день 3 (без 🔒 на пути) стянуто к «С» (не держит день)');
