...
// ── 3) «Срок изготовления» (EDD) НЕ привязывает день; #3980 — но задаёт ПОРЯДОК при равной
//       переналадке ──
// Один и тот же вход, с dueKeyByCut и без. A и B — разное сырьё, одни ножи (переход 15 в любом
// порядке, стоимость равна). Без сроков порядок по id (A, B). Со сроками (#3980) более ранний
// срок B (01.07 < 10.07) встаёт РАНЬШЕ A — просроченная/срочная не после дальней. При этом день
// НЕ меняется: обе резки по-прежнему в дне 0 (срок не отдаёт ранний день, day-anchor #3658 снят).
var base3 = [ cut('A', 'matA', 2, 0), cut('B', 'matB', 2, 0) ];
var r3a = plan(base3);
var r3b = plan(base3, { dueKeyByCut: { A: 20260710, B: 20260701 } });
assertEqual(r3a.order, ['A', 'B'],
    '#3974: без сроков порядок — по id (A, B)');
assertEqual(r3b.order, ['B', 'A'],
    '#3980: dueKeyByCut задаёт порядок при равной переналадке — ранний срок B раньше A');
assertEqual([r3b.day.A, r3b.day.B], [0, 0],
    '#3974: срок НЕ привязывает день — обе резки в дне 0 (day-anchor #3658 остаётся снят)');
...
// ── 7) Фольга — в конец дня (#3717 сохранён без EDD) ──
// N1, N2 (нефольга) и F1 (фольга), одно окно. Фольга размещается ПОСЛЕ обычных резок.
var r7 = plan([ cut('N1', 'matA', 1, 0), cut('N2', 'matA', 1, 0), cut('F1', 'matA', 1, 0, { foil: true }) ]);
assertEqual(r7.order[r7.order.length - 1], 'F1',
    '#3974: фольга (F1) размещается в конец дня — после нефольги (#3717)');
