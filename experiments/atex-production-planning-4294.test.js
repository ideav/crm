...
(function () {
    var cuts = [
        // #4561: фикс 21.07 — исключаем ТАК ЖЕ, как незафиксированный. Прежде считалось, что «его
        // держит движок (fixedDay<0)», но движок его не держит, а ПРОПУСКАЕТ, и занятость такого
        // задания исчезала из дня. Исключённое из входа задание остаётся на своём месте И занимает
        // окно станка (excludedCutBlockedRanges) — для оператора итог тот же, для дня — честный.
        scut('Ffix', D21, { fixed: true }),
    ...
    eq(keep(cuts, BASE), ['Ffix', 'Ufree', 'Uold'],
        '#4294/#4561: исключаем ВСЁ раньше «С» — и фикс тоже; в-окне/новые остаются');
})();
...
// ── Цепочка с ЗАФИКСИРОВАННОЙ головой раньше «С» — исключаем ЦЕЛИКОМ (#4561) ─────────────────────────
// Замок день головы не меняет: она в прошлом, планировать её нечего. Прежнее «не исключаем — движок
// держит» опиралось на неверную посылку: splitMachineQueue кладёт задания только вперёд от дня 0 и
// якорь < 0 НЕ размещает вовсе, поэтому хвост цепочки, стоящий в окне, выпадал из занятости дня —
// упаковщик набивал день поверх него (issue #4561, боевое 01.08.2026: станко-день 479 при потолке 455).
(function () {
    var cuts = [scut('Hf', D21, { fp: 'Hf', fixed: true }), scut('Cf', D22, { fp: 'Hf' })];
    eq(keep(cuts, BASE), ['Hf', 'Cf'], '#4561: цепочка с ЗАФИКСИРОВАННОЙ головой раньше «С» исключается ЦЕЛИКОМ');
})();
