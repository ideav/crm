...
(async function() {
    ...
    // #4211: при НАЛИЧИИ просрочки (опозданий 1 > 0) сообщение НЕ должно говорить «оптимальна».
    assert(!/оптимальна/.test(s2.notify) && /опозданий 1/.test(s2.notify),
        '#4211: none c просрочкой (опозданий 1) → НЕ «оптимальна», а «просрочка не устранена» (' + s2.notify + ')');
    // Контроль: БЕЗ просрочки (опозданий 0) «уже оптимальна» сохраняется.
    var s2b = await runScenario({ co: [480, 500], late: [0, 0] });
    assert(s2b.applySplit === 0 && /оптимальна/.test(s2b.notify),
        '#4211 контроль: none БЕЗ просрочки (опозданий 0) → «уже оптимальна» (' + s2b.notify + ')');
