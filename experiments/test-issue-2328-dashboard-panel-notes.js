...
// Важно, что отступ снизу ЕСТЬ (заметки не липнут к содержимому панели); конкретная величина —
// вопрос оформления и правится дизайном.
assert(/\.f-panel-notes\s*\{[^}]*margin-bottom:\s*(?!0[^.\d])[\d.]+rem;/.test(css),
    'dashboard notes must leave a margin below');
