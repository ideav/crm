...
// ---------------------------------------------------------------------------
// Часть 1c — firstPartId с пробелами (как приходит из rowValue). chainRoot нормализует String().trim()
// ТАК ЖЕ, как группировка цепочек #3892, иначе голова (сравнение по id) и продолжение (по firstPartId
// с пробелами) считались бы РАЗНЫМИ цепочками → ножи не выносятся → день раздут. Ровно этот рассинхрон
// оставлял 26.06 на 492 на реальном плане (продолжение не распознавалось при строгом ===).
...
var HEADw = cut('HEADw', 0, 0,   'MR314L', [200], 'B_314',  sec(2026, 5, 26, 15, 35), '');
...
// ---------------------------------------------------------------------------
// Часть 1d — между хвостом и продолжением стоит резка ЧУЖОЙ цепочки: ножи НЕ выносим (у соседней своя
// переналадка от нового сырья — иначе двойной счёт). Продолжение = только БЛИЖАЙШАЯ резка той же цепочки.
console.log('\n== #4030: чужая резка между хвостом и продолжением — ножи НЕ выносим (нет двойного счёта) ==');
var FILLx = cut('FILLx', 3, 300, 'OTHER',  [100], 'B_OTHER', sec(2026, 5, 26, 8, 0),  '');
var HEADx = cut('HEADx', 0, 0,   'MR314L', [200], 'B_314',   sec(2026, 5, 26, 15, 35), '');
var OTHRx = cut('OTHRx', 3, 40,  'ZZZMAT', [500], 'B_ZZZ',   sec(2026, 5, 29, 8, 0),  '');           // чужая цепочка ПЕРВОЙ
var CONTx = cut('CONTx', 3, 9,   'MR314L', [200], 'B_314',   sec(2026, 5, 29, 8, 30), 'HEADx');       // продолжение — не соседнее
...
eq((byX['HEADx'] ? byX['HEADx'].knife + byX['HEADx'].material : -1), 45,
   'HEADx: чужая резка следом → настройка целиком 45 на хвосте (ножи не выносим, иначе двойной счёт у соседа)');

// ---------------------------------------------------------------------------
// Часть 1e — firstPartId ПУСТ у продолжения (как на ateh: в отчёте cut_planning нет колонки
// cut_first_part → firstPartId всегда ∅ для ВСЕХ резок). Продолжение опознаётся по НУЛЕВОЙ переналадке
// входа (та же конфигурация — ножи хвоста и есть заправка продолжения), а не по цепочке firstPartId.
// Ровно этот случай (#4039, лог PP_TRACE: «продолжение НЕ распознано … fp=∅») оставлял 26.06 на 492.
...
var HEADe = cut('HEADe', 0, 0,   'MR314L', [200], 'B_314',   sec(2026, 5, 26, 15, 35), '');   // fp ∅
