...
define("DB_MASK", "/^[a-z0-9_]\w{1,14}$/i");  # Mask for the DB name validation
...
if(!preg_match("/^[a-z0-9_]\w{1,14}$/i", $z)&&($z!=="auth.asp"))
...
if($_SERVER["REQUEST_URI"] === "/alex?trace"){
...
if(($z === "my") && ((isset($com[2]) ? $com[2] : "") === "register")){ # Register the user
    ...
    if(isset($_GET["c"]) && isset($_GET["u"])){
    	...
    	$u = (int)$_GET["u"];
    	if($u > 0){
        	$result = Exec_sql("SELECT user.val user, user.id uid, token.id tok, xsrf.id xsrf, act.id act, pwd.val pwd, pwd.id pid, email.val email
...
elseif(($z == "auth.asp") && !empty($_GET['code'])){
    # potok4hr@gmail.com
    ...
    if (!empty($data['access_token'])) {
        ...
        if($row = mysqli_fetch_array(Exec_sql("SELECT user.id uid, token.id tok, token.val tokval, xsrf.id xsrf, act.id act, db.val db
                                        FROM $z user LEFT JOIN $z token ON token.up=user.id AND token.t=".TOKEN
                                                ." LEFT JOIN $z xsrf ON xsrf.up=user.id AND xsrf.t=".XSRF
                                                ." LEFT JOIN $z act ON act.up=user.id AND act.t=".ACTIVITY
...
elseif(($z === "api") || ($_SERVER["REQUEST_METHOD"] == "OPTIONS")){
...
function isApi(){
    global $dumpAPI;
    return (isset($dumpAPI) || isset($_POST["JSON"]) || isset($_GET["JSON"]) || isset($_POST["JSON_DATA"]) || isset($_GET["JSON_DATA"]) || isset($_POST["JSON_KV"]) || isset($_GET["JSON_KV"]) || isset($_POST["JSON_CR"]) || isset($_GET["JSON_CR"]) || isset($_POST["JSON_T"]) || isset($_GET["JSON_T"]) || isset($_POST["JSON_HR"]) || isset($_GET["JSON_HR"]));
}
function xsrf($a, $b){
...
function HintNeeded($k, $id, $par)
{
    if(isset($_REQUEST["USEINDEX_$par"]))
        return $_REQUEST["USEINDEX_$par"] === "1";
...
function Construct_WHERE($key, $filter, $cur_typ, $join_req=0, $ignore_tailed=FALSE)
{
	...
	foreach($filter AS $f => $value)
	{
		...
		elseif((substr(trim(strtoupper($value)), 0, 3) == "IN(") && substr(trim($value), -1) == ")") # Multiple options - IN()
        {
            ...
            $sql_value = Escape_Filter_Val($value);
		...
		{ # If we have a [substitute], don't add '' — it is substituted later.
		  # Готовым выражением считаем только значение, закавыченное ЦЕЛИКОМ. Раньше от
		  # экранирования освобождалось любое значение, ВНУТРИ которого есть апостроф, —
		  # из-за этого имя вида "Yan Pol' Musique" роняло запрос, а фильтр F_ из адресной
		  # строки подставлялся в SQL как есть.
			$quoted = (strlen($value) > 1) && (substr($value, 0, 1) === "'") && (substr($value, -1) === "'");
			$v = (preg_match("/\[([^\[\]]+)\]/", $value) || $quoted) ? $value : "'".addslashes($value)."' ";
		...
		switch($GLOBALS["REV_BT"][$key])
		{
			...
			case "HTML":
				...
				elseif(((mb_strlen($value) <= VAL_LIM) && (strpos($value, "%") === FALSE)) # No tail & index is possible, just like SHORT
							|| $ignore_tailed)	# Or we treat all tailed as SHORT
				{
					if($key == $cur_typ)	# Filter was applied to the Object's Value
						$GLOBALS["where"] .= " AND vals.val $search_val ";
					else
						...
						&& ((substr($value, 0, 1) != "_") || (strpos($value, "%") === FALSE))) # It's not a LIKE condition
				{
					...
					if(strpos($value, "%") === FALSE)
						$short_search_val = "$NOT_EQ='".Escape_Filter_Val(mb_substr($value, 0, VAL_LIM))."'";
					else
						$short_search_val = "$NOT LIKE '".Escape_Filter_Val(mb_substr($value, 0, min(mb_strpos($value, '%', 0), mb_strpos($value, '%', 0))))."%'";
...
#print_r($GLOBALS);print_r($blocks);die("$"."f = ".$f);
				...
				if((!isset($filter["TO"])) || (!isset($filter["FR"])))
				{
					...
					{
						if($NOT_flag) # No match or empty
							$GLOBALS["where"] .= " AND (a$key.val$NOT_EQ='$sql_value' OR a$key.val IS NULL)";
						else
				...
				if((!isset($filter["TO"])) || (!isset($filter["FR"]))){
					if($key == $cur_typ)
					{
						...
						elseif(strpos($value, "%") === FALSE)
							$GLOBALS["where"] .= " AND vals.val$NOT_EQ='$sql_value' ";
						else
...
function Check_Types_Grant($fatal=TRUE)	# $fatal stops the script on no access
{
	...
	if($fatal)
		die(t9n("[RU]У вас нет прав на редактирование и просмотр типов ([EN]You do not have the grant to view and edit the metadata (")
...
function Validate_Token() # Validates the cookie token and gathers the user permissions
{
	...
	elseif(isApi())
	{
    		...
    		elseif(strtolower($key) === "authorization"){
    		    if(strtolower(substr($value, 0, 5)) === "basic"){  // Basic HTML authorization
            		...
            		if($row = mysqli_fetch_array($data_set)){
            		    ...
            		    die("[{\"error\":\"Invalid login/password (login:$u)\"}]");
                	}
    		    }
		...
		if(!isset($tok)){
	        header("HTTP/1.0 401 Unauthorized");
		    die("[{\"error\":\"Basic auth: No authorization token provided\"}]");
		}
	}
...
#print_r($GLOBALS); die($v." ".$attrs." ".count($blocks[$attrs][strtolower($attrs)]));
		...
		elseif(isset($_COOKIE[$z]) || ($tok == ADMINHASH))
		    if(($_COOKIE[$z] == ADMINHASH)  || ($tok == ADMINHASH))
    		{
	...
	{ # Check, if we got a Guest user defined
		$data_set = Exec_sql("SELECT u.id u, tok.val tok, xsrf.id xsrf, role_def.id r FROM $z u LEFT JOIN $z tok ON tok.up=u.id AND tok.t=".TOKEN
								." LEFT JOIN $z xsrf ON xsrf.up=u.id AND xsrf.t=".XSRF
...
#		    die("SELECT u.val FROM my tok, my u, my db"
			...
			if($row["req_t"])	# Do we have Reqs?
			{
				...
				elseif($row["arr"])
				{
					...
					if(!isset($GLOBALS["local_struct"][$row["req_t"]]))
					{
						...
						$GLOBALS["arraysById"][$id][$row["arr"]] = $row["req_t"];
					}
				}
				else
				{
...
function Compile_Report($id, $cur_block, $exe=TRUE, $check=FALSE) # $exe means we must retrieve data at last, not just prep the sql
{
...
# Get the Entities - those having others as requisites
		if(!isset($GLOBALS["STORED_REPS"][$id]["sql"]))
		{
			$sql = "SELECT distinct CASE WHEN col_def.up=0 THEN col_def.id ELSE col_def.up END typ, reqs.id req, req_refs.t refr, arr_vals.up arr, reqs.val attr, rep.ord
...
#                            		    $join = " AND " . preg_replace("/:(\d+):/", "$1", $rf[REP_JOIN_ON]);
							...
							if($typ != $par)	# We are a requisite
							{
								...
								if($l = isRef($id, $par, $typ)){
									if(HintNeeded($key, $id, $alias)) // multiple refs to fix
									{
...
#	    							$field_names[$key] = "CONCAT($p$alias.id, SUBSTRING($field, -4)) $name";
							...
							if(!isset($field_names[$key])){
								...
								if(strpos($tailed, $GLOBALS["STORED_REPS"][$id]["base_in"][$key]) && $exe) // Do not fetch tails for sub-queries
...
# Fill in the field set with built-in and request values, if any
			...
			if(isset($order))
			{
    			...
    			preg_match_all("/(\[[0-9a-zA-Z\_]+\])/ims", $sortByArr, $builtins);
    			foreach($builtins[0] as $builtin)
    				if(BuiltIn($builtin) != $builtin)
...
# Save the Report, it might be required again this session
    ...
    if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_FUNC][1]) && ($GLOBALS["STORED_REPS"][$id][REP_COL_FUNC][1] === "RECURSIVE")){
        ...
        if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_FROM][1])){
            $sub_query = substr($GLOBALS["STORED_REPS"][$id][REP_COL_FROM][1], 1, -1);
			$bak_id = $id;
			Get_block_data($sub_query, FALSE); # Just get the SQL, no execution
			$id = $bak_id;
			if(isset($GLOBALS["STORED_REPS"][$GLOBALS["STORED_REPS"][$sub_query]["_rep_id"]]["sql"])){
                $cond = "IN(".$GLOBALS["STORED_REPS"][$GLOBALS["STORED_REPS"][$sub_query]["_rep_id"]]["sql"].")";
        		if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_NAME][1]))
        			$name = "'".str_replace("'", "\\'", $GLOBALS["STORED_REPS"][$id][REP_COL_NAME][1])."'";
        		else
        			$name = "id";
        		$GLOBALS["STORED_REPS"][$id]["sql"] = " WITH RECURSIVE rec AS (SELECT id, 0 t FROM $z WHERE id $cond"
                        .   " UNION ALL"
                        .   " SELECT ref.up id, ref.t FROM $z ref INNER JOIN rec ON rec.id=ref.t WHERE ref.val='$typ')"
                        ." SELECT DISTINCT id $name FROM rec";
			}
        }
	}
...
#	if(isset($GLOBALS["TRACE"]))
#	    mywrite($sql);
	if(isset($_REQUEST["EXPLAIN"]) && ($_REQUEST["EXPLAIN"] === "1")){
	    trace("EXPLAIN requested");
    	$data_set = Exec_sql("EXPLAIN ".$sql, "Request report EXPLAIN", TRUE, FALSE);
    	$h = "<table>";
        while($row = mysqli_fetch_assoc($data_set))
            $h .= "<tr><td style='border: 1px solid gray'>".implode("<td style='border: 1px solid gray'>", $row)."\r\n";
    	die($h."</table><br>\r\n$sql");
	}
	if(isset($_REQUEST["RECORD_COUNT"]))
	{
			...
			if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_FUNC][$key]))
				switch($GLOBALS["STORED_REPS"][$id][REP_COL_FUNC][$key]) # Internal functions are named abn_XXXX
				{
					...
					case "abn_URL":
						...
						$post = isset($GLOBALS["STORED_REPS"][$id]["rep_params"]["REP_POST"]) ? $GLOBALS["STORED_REPS"][$id]["rep_params"]["REP_POST"] : "";
						if(isset($GLOBALS["STORED_REPS"][$id]["rep_params"]["URL_HEADER"]))
						    $headers = explode(",", $GLOBALS["STORED_REPS"][$id]["rep_params"]["URL_HEADER"]);
						else
						    $headers = array("User-Agent: Integral");
...
#			trace("Format $val base:$base - base_str:$base_str id:".$row["i$key"]);
    		...
    		if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_FORMULA]))
    		    if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_FORMULA][$key]))
    		        $val = checkJson($GLOBALS["STORED_REPS"][$id][REP_COL_FORMULA][$key], $val);
	    ...
	    elseif(isset($_REQUEST["JSON_KV"])){
    		...
    		foreach($GLOBALS["STORED_REPS"][$id]["head"] as $key => $value)
        	    if(isset($field_names[$key])) # Only those values selected
        		{
        		    ...
        		    $json["columns"][$i]["id"] = $GLOBALS["STORED_REPS"][$id]["columns"][$key];
        		    $json["columns"][$i]["type"] = $GLOBALS["STORED_REPS"][$id]["types"][$key];
        		    $json["columns"][$i]["format"] = $GLOBALS["STORED_REPS"][$id]["base_out"][$key];
        		    $json["columns"][$i]["name"] = $value;
        		    if(isset($GLOBALS["STORED_REPS"][$id]["ref_typ"][$GLOBALS["STORED_REPS"][$id]["types"][$key]]))
...
function checkJson($jsonKey, $val){
    if(strtoupper(substr($jsonKey, 0, 5)) === "JSON." && mb_strpos(strtoupper($val),strtoupper(substr($jsonKey, 5))) !== FALSE){
        $tmp = json_decode($val, TRUE);
        $keyName = substr($jsonKey, 5);
        array_walk_recursive($tmp, function ($v, $k) use (&$val, &$keyName){
            if($k === $keyName)
                return $val = $v;
        });
    }
	...
	switch ($block_name)
	{
		...
		case "&main":
			...
			switch($GLOBALS["GLOBAL_VARS"]["action"]) # Show the current mode & object in the page header
			{
				case "object":
					if($id == 0)
						die(t9n("[RU]Ошибка: id=0 или не задан / не найден[EN]Object id is empty/not found or 0"));
        			...
        			while($row = mysqli_fetch_array($data_set))  # All but buttons and calculatables
        				if(($GLOBALS["REV_BT"][$row["t"]] != "CALCULATABLE") && ($GLOBALS["REV_BT"][$row["t"]] != "BUTTON"))
        				{
        					...
        					if(!isset($req[$i]) && !isset($typ[$i])){ # Not used as Req yet
        					    ...
        					    $reqs[$i] = Array();
        					    $base[$i] = $row["base"];
                				if(in_array($GLOBALS["REV_BT"][$base[$i]], array("CHARS", "MEMO", "FILE", "HTML"))){
...
//#[AS]07.01.2019				, CASE WHEN ref_typs.id!=ref_typs.t THEN ref_typs.val ELSE req_typs.val END req_val
		...
		case "&object":
			...
			if($row = mysqli_fetch_array($data_set))
			{
				...
				$blocks[$block]["up"][] = $GLOBALS["parent_id"] = $GLOBALS["GLOBAL_VARS"]["up"] = $row["up"];
...
#print_r($GLOBALS);die();
				$data_set = Exec_sql("SELECT pars.id par_id, reqs.id req_id, pars.val par_name, pars.t par_base
     			...
     			if(isApi()){ # Prepare the list of dependant terms
...
#						$v = Get_tail($row["id"], $v);
				...
				else  # No requisite yet - add the default value, if any
				{
					...
					{
						$attrs = str_replace(NOT_NULL_MASK, "", $row["attrs"]); # Remove NOT_NULL, MULTI, and ALIAS by mask
				...
				if(isset($row["ref_req"]))
				{
					...
					if(isset($search_arr[$req_count]))	# Apply Req's filters, if any
						if(strlen($search_arr[$req_count]))
						{
							...
							$GLOBALS["where"] = "";
							Construct_WHERE($req, array("F" => $search_arr[$req_count]), 1, FALSE, FALSE);
...
#die("$req $cur_ref_typ ".$row["ref_req"]);
			...
			trace(" cur ref val ".$blocks[$blocks[$block]["PARENT"]]["CUR_VARS"]["val"]);
				...
				if($plain_data){
    				...
    				while(!feof($handle)){
    					...
    					if($getParent){
    					    ...
    					    $parCond = addcslashes(Format_Val($parentBase,UnMaskDelimiters($object[0])), "\\\'");
    					    if(isset($_REQUEST["noorder"]))
        					    $sql = "SELECT par.id, 0 ord FROM $z par WHERE par.t=$parentType AND (par.val='$parCond' $orid)";
        				    else
        					    $sql = "SELECT par.id, max(reqs.ord) ord FROM $z par LEFT JOIN $z reqs ON reqs.up=par.id AND reqs.t=$id "
        					                ."WHERE par.t=$parentType AND (par.val='$parCond' $orid)  HAVING par.id IS NOT NULL";
    					...
    					trace("base ".$GLOBALS["base"][$id]);
    					$object[0] = Format_Val($GLOBALS["base"][$id],UnMaskDelimiters($object[0]));
    					if($isUnique && ($lastVal === $object[0]))
    					    continue;
    					$lastVal = $object[0];
					    $reqs = Array();
					    $ids = Array();
					    if(isset($GLOBALS["arraysById"][$id]))
    					    $arrs = "AND reqs.t NOT IN(".implode(",", $GLOBALS["arraysById"][$id]).")";
    					else
    					    $arrs = "";
    					if($isUnique){
    					    $result = Exec_sql("SELECT obj.id, reqs.t, reqs.val, reqs.id reqid
    					                            FROM $z obj LEFT JOIN $z reqs ON reqs.up=obj.id AND reqs.t!=0 $arrs
            			    ...
            			    if($row = mysqli_fetch_array($result)){
        				        ...
        				        do{
        				            if($existing !== $row["id"]) // There may be several matches, take only the first one
        				                break;
        				            if($row["t"]){
            							if(in_array($GLOBALS["REV_BT"][$GLOBALS["base"][$row["t"]]], array("CHARS", "MEMO", "FILE", "HTML")))
                        			        $reqs[$row["t"]] = Get_tail($row["reqid"], $row["val"]);
                        			    else
                        			        $reqs[$row["t"]] = $row["val"];
                    			        $ids[$row["t"]] = $row["reqid"];
			...
			foreach($GLOBALS["REQS"] as $req => $base)
				if(isset($GLOBALS["GRANTS"]["mask"][$req]))
				{
					foreach($GLOBALS["GRANTS"]["mask"][$req] as $mask) # Apply all masks
					{
						...
						Construct_WHERE($req, array("F" => $mask), $cur_typ, $req, FALSE);
...
#				$filter_cond = " and vals.id IS NOT NULL"; 
			...
			elseif(!isset($_REQUEST["csv"]) && !isset($_REQUEST["bki"]))	# Retrieve all data in case of XLS export
			...
			if(isset($_GET["_count"])){
    			...
    			api_dump(json_encode(array("count"=>$row["cnt"])), "count.json");
			}
			$data_set = Exec_sql("SELECT $distinct vals.id, vals.t, vals.val, vals.up $vals_ord ". (isset($tails) ? $tails : "")
...
#{print($head_str);}#print_r($GLOBALS["data"]);print_r($GLOBALS);die();}
				...
				&& strlen($filter_cond))
			{
				...
				{
					trace(" filter_tables $filter_tables");
					if($row = mysqli_fetch_array(Exec_sql("SELECT COUNT($distinct vals.id) cnt FROM $z vals $filter_tables
...
#		case "&head_move_n_delete":
		...
		case "&page": # Construct the Pages navigation bar
			if((isset($blocks[$blocks[$block]["PARENT"]]["CUR_VARS"]["pages"]) ? $blocks[$blocks[$block]["PARENT"]]["CUR_VARS"]["pages"] : 0) > 1){
		...
		case "&uni_object_view_reqs":
			...
			if(isset($GLOBALS["HAVE_ARR"]))
				$sql = "SELECT CASE WHEN typs.up=0 THEN 0 ELSE reqs.id END id, CASE WHEN typs.up=0 THEN 0 ELSE reqs.val END val
							, reqs.ord, typs.id t, typs.up, typs.val refr, count(1) arr_num";
			else
				$sql = "SELECT reqs.id, reqs.val, typs.id t, typs.up, typs.val refr, reqs.ord";
			...
			while($row = mysqli_fetch_array($data_set))
				if(isset($GLOBALS["NonREF_typs"][$row["t"]])){
					...
					$rows[$row["val"]]["ord"][] = $row["ord"];
					$rows[$row["val"]]["ref_id"][] = $row["t"];
				}
			#trace(print_r($GLOBALS));
			foreach($GLOBALS["REQS"] as $key => $value){
#print_r($GLOBALS);print_r($rows);die();
...
#print_r($row);
				...
				if(isApi()){
					...
					elseif($literal_typ == "BUTTON")
					    ...
					    $v = $val;
    			    if($v != "")
    					$GLOBALS["GLOBAL_VARS"]["api"]["reqs"][$parent_id][$key] = is_array($v)?implode(",", $v):$v;
    			}
				if(isset($_REQUEST["csv"])){
					...
					elseif($literal_typ == "BUTTON"){
						$tmp = str_replace(NOT_NULL_MASK, "", $GLOBALS["attrs"][$key]); # Remove NOT_NULL, MULTI, and ALIAS by mask
						$tmp = str_replace(MULTI_MASK, "", $tmp);
						$tmp = preg_replace(ALIAS_MASK, "", $tmp);
...
#{print_r($GLOBALS);print_r($rows);print_r($blocks[$block]);die();}
		...
		case "&buttons":
			if(isset($blocks["BUTTONS"]))
				...
				{
					$value = str_replace(NOT_NULL_MASK, "", $value); # Remove NOT_NULL, MULTI, and ALIAS by mask
					$value = str_replace(MULTI_MASK, "", $value);
					$value = preg_replace(ALIAS_MASK, "", $value);
...
#print_r($GLOBALS);
		...
		case "&settings":
			...
			while($row = mysqli_fetch_array($data_set)){
				...
				$blocks[$block]["settings"][] = Get_tail($row["tails"], $row["settings"]);
			}
			break;
		...
		default:
			...
			{
				Compile_Report($rep_id, $block, $exe);
				if(!$exe)
					return;
				$bak_id = $id;
				$id = $rep_id;
...
function getRefObjID($refType, $ref){
	global $z;
...
{
	global $connection, $z;
	exec_sql("INSERT INTO $z (up, ord, t, val) VALUES ($up, $ord, $t, '".addcslashes(mb_substr($val, 0, VAL_LIM), "\\\'")."')", "Insert: $message");
    $id = mysqli_insert_id($connection);
	$ord = 0; # Add the tail if applicable
	while(mb_strlen($val) > VAL_LIM)
	{
    	$val = mb_substr($val, VAL_LIM); # Cut off the rest of the value
    	exec_sql("INSERT INTO $z (up, ord, t, val) VALUES ($id, ".($ord++).", 0, '".addcslashes(mb_substr($val, 0, VAL_LIM), "\\\'")."')"
    	        , 'Save tail $id ($order)');
	}
	return $id;
}
...
function verifyJWT($jwt, $publicKey) {
    ...
    $jsonPayload = json_decode($payloadDecoded, true);
	if(json_decode($headerDecoded, true)['x5u'] && (strpos(json_decode($headerDecoded, true)['x5u'],"localhost") === false)){
	    $x5u = json_decode($headerDecoded, true)['x5u'];
        $publicKey = file_get_contents($x5u);
	}
    ...
    if($verified === 1){
        $now = time();
        if(!array_key_exists('iat', $jsonPayload) || !array_key_exists('exp', $jsonPayload))
...
#		setcookie($z, $GLOBALS["GLOBAL_VARS"]["token"], time() + COOKIES_EXPIRE, "/"); # 30 days
		setcookie($z, $GLOBALS["GLOBAL_VARS"]["token"], 0, "/"); # Upon browser close
...
Exec_sql("SET SESSION sql_mode = 'STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION'", "sql_mode");
...
switch($a)  # Check actions, which don't require authentication
{
	...
	case "auth":
		...
		$data_set = Exec_sql("SELECT u.id uid, u.val, pwd.id pwd_id, pwd.val pwd, tok.id tok, act.id act, xsrf.id xsrf
...
#			else
		...
		elseif((strtolower($u) == "admin") && (sha1($p) == ADMINHASH)){
			$GLOBALS["GLOBAL_VARS"]["user"] = $GLOBALS["GLOBAL_VARS"]["role"] = "admin";
			$GLOBALS["GLOBAL_VARS"]["user_id"] = 0;
			$GLOBALS["GLOBAL_VARS"]["xsrf"] = xsrf(ADMINHASH, "admin");
			$GLOBALS["GLOBAL_VARS"]["token"] = ADMINHASH;
			setcookie($z, ADMINHASH, 0, "/");
		}
		elseif(isApi())
...
#die("$t : $v != ".$GLOBALS["REQS"][$t]);
					if($t == $typ)
					{
            	        ...
            	        if(($typ == USER) && ($GLOBALS["REQS"][$t] === $z))
							my_die(t9n("[RU]Нельзя изменить имя администратора, лучше создайте нового[EN]Please create another user instaed of renaming the admin"));
					}
...
#					echo("The same parent $up");
		...
		case "_m_new":
			...
			$data_set = Exec_sql("SELECT obj.t, obj.ord, req.id, req.t reqt, req.val FROM $z obj LEFT JOIN $z req ON req.up=$id WHERE obj.id=$id AND obj.up=0"
								, "Check Obj type&reqs");
			if($row = mysqli_fetch_array($data_set))
			{
			    ...
			    do{
			        ...
			        if(!isset($_REQUEST["t".$row["id"]])&&!isset($_REQUEST["NEW_".$row["id"]])){
    					$attrs = str_replace(NOT_NULL_MASK, "", $row["val"]); # Remove NOT_NULL, MULTI, and ALIAS by mask
    					$attrs = str_replace(MULTI_MASK, "", $attrs);
    					$attrs = preg_replace(ALIAS_MASK, "", $attrs);
    					if($attrs !== ""){
    						$v = BuiltIn($attrs); # Calc predefined value
...
# Type editor commands
		case "_d_req":
			...
			if($row = mysqli_fetch_array(Exec_sql("SELECT obj.up objup, new.t, obj.val, new.id, new.up FROM $z obj LEFT JOIN $z new ON new.id=$t WHERE obj.id=$id"
							                , "Check the new req")))
			{
...
#print_r($GLOBALS); die($sql);
		...
		case "metadata":
	        ...
	        foreach($meta as $k => $m)
	            if($metaReqs[$k])
    	            ...
    	            $meta[$k] = "{".$m.",\"reqs\":[]}";
			api_dump("[".implode(",", $meta)."]", "metadata_".($isOne ? "$id" : "all").".json");
		    break;
		    
		case "xsrf":
			api_dump(json_encode(array("_xsrf"=>$GLOBALS["GLOBAL_VARS"]["xsrf"],"token"=>$GLOBALS["GLOBAL_VARS"]["token"],"user"=>$GLOBALS["GLOBAL_VARS"]["user"]
			    ,"role"=>$GLOBALS["GLOBAL_VARS"]["role"],"id"=>$GLOBALS["GLOBAL_VARS"]["user_id"],"msg"=>"")), "login.json");
		    break;
		...
		case "_ref_reqs":
			...
			$data_set = Exec_sql("SELECT dic.t dic, def_reqs.t, req_orig.t base, base.id!=base.t is_ref, req.id req_id
				...
				if(isset($row["t"])){
        			...
        			if(isset($_GET["r"]) && ($restrict === "")){
        			    ...
        			    elseif(is_numeric($_GET["r"]))
        				    $restrict = " AND a$req.id=".$_GET["r"];
        				trace("restrict = $restrict");
        			}
				}
			}
		    trace("wild_search $wild_search");
			if(isset($_GET["q"]) && strlen($_GET["q"])){	# Apply Req filter, if any
    		    $wild_search = $wild_search === "" ? "vals.val" : "concat(vals.val $wild_search)";
				$GLOBALS["REV_BT"][1] = "SHORT";
				$GLOBALS["where"] = "";
				if(strpos($_GET["q"],"%") === false)
    				$search = preg_replace("( ?\/ ?)","%/%",$_GET["q"]);
				else
				    $search = $_GET["q"];
				Construct_WHERE(1, array("F" => "%".addslashes($search)."%"), 1, FALSE, FALSE);
				trace("wild_search now $wild_search");
				trace("search_req ".$GLOBALS["where"]);
				$search_req = str_replace("vals.val", $wild_search, $GLOBALS["where"]);
			}
		    trace("search_req now $search_req");
			# Fetch grants
			if(isset($GLOBALS["GRANTS"]["mask"])){
    			# Fetch grants to the Referenced Object itself
    			if(isset($GLOBALS["GRANTS"]["mask"][$dic])){
    				unset($granted);
    				foreach($GLOBALS["GRANTS"]["mask"][$dic] as $mask){ # Apply all masks
    					$GLOBALS["where"] = "";
    					$GLOBALS["REV_BT"][$dic] = "SHORT";
    					Construct_WHERE($dic, array("F" => $mask), 1, FALSE, TRUE);
    					if(isset($granted))
    						$granted .= " OR ".substr($GLOBALS["where"], 4);
    					else
    						$granted = substr($GLOBALS["where"], 4);
