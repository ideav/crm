...
// ---- rule 3: обфускация сумм — 3 знака ДО запятой (value mod 1000), детерминированный rng ----
assert.strictEqual(OBF.obfuscateSum('1000', () => 0), '500');        // 1000×0.5=500 -> %1000=500
assert.strictEqual(OBF.obfuscateSum('1234567', () => 0), '283.5');   // 617283.5 -> %1000=283.5 (3 знака + дробь)
assert.strictEqual(OBF.obfuscateSum('2000', () => 0.5), '500');      // 2000×7.75=15500 -> %1000=500
assert.strictEqual(OBF.obfuscateSum('1', () => 1 / 7), '2.57');      // 2.5714… -> округление до 2 знаков
assert.strictEqual(OBF.obfuscateSum('1 234,50', () => 0), '617.25'); // пробел + запятичная дробь
assert.strictEqual(OBF.obfuscateSum('0', () => 0), '0');             // ноль — как есть
assert.strictEqual(OBF.obfuscateSum('нет', () => 0), 'нет');         // не число — как есть

// инвариант: не больше 3 знаков ДО запятой (|n| < 1000) и не больше 2 знаков после
['3.14159', '99999.99999', '7', '0.123456', '888888.5', '-54321.99'].forEach(function(v){
    [0, 0.5, 1 / 3, 0.999].forEach(function(rv){
        var out = OBF.obfuscateSum(v, () => rv);
        assert.ok(Math.abs(parseFloat(out)) < 1000, '≤3 знака до запятой: ' + v + ' -> ' + out);
        var dot = out.indexOf('.');
        assert.ok(dot === -1 || (out.length - dot - 1) <= 2, '≤2 знака после запятой: ' + out);
    });
});
