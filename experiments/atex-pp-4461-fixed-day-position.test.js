...
// ── 2. Цена разрыва: день 150 → 120 мин переналадки ──────────────────────────────────────
(function () {
    var cuts = day29();
    var wedged = ['647845', '647159', '646483', '646890', '646822'];   // как было на боевой
    var got = order(pack(cuts));
    assert(changeover(cuts, wedged) === 150, 'боевой порядок стои́т 150 мин переналадки',
        '(' + changeover(cuts, wedged) + ')');
    assert(changeover(cuts, got) === 120, 'порядок §8 стои́т 120 мин — на смену ножей меньше',
        '(' + changeover(cuts, got) + ')');
})();
...
// ── 5. НИЧЬЯ по переналадке остаётся за 🔒 (прежнее поведение #3792 / прикол 🗓 #4221) ────
// С заправки (MWR200 110/55) и свободная MR194 30×29, и 🔒 MW308 IN 150/59 стоят одинаково
// (смена ножей + смена сырья). Дешевле не значит «раньше»: при равной цене вперёд идёт 🔒.
(function () {
    ...
    assert(got.join(' → ') === 'lock → free', 'при равной переналадке 🔒 берётся первой',
        '(' + got.join(' → ') + ')');
})();
