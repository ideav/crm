...
// ── G (#4506): «По весу» + галка — место по ВЕСАМ через замок дня, соседи-🔒 не съезжают ─────
// Так теперь и приходит боевой путь: moveCutToDay при fix=true отдаёт задание и в wholeDayByCut
// (ложится в день целиком, #4488), и в dayLockByCut (позицию в дне выбирают веса, #4506), а ДЕНЬ
// держит фикс-якорь. Проверяем: X садится рядом со своим близнецом по ножам, а зафиксированный
// сосед B остаётся на своём месте в дне (issue #4506: 🔒 уезжали в конец дня).
(function () {
    var cuts = [
        cut('A', 'MR194',  'OUT', K([[40, 22]]),  40,   0),
        cut('B', 'MWR118', 'OUT', K([[110, 8]]), 110, 600, true),   // 🔒 хранимая, середина дня
        cut('C', 'MR194',  'OUT', K([[25, 35]]),  25, 1200),
        movedCut(-60, true)                                          // X: «По весу» + «Зафиксировать»
    ];
    var ops = runPlan(cuts, { wholeDayByCut: { X: 0 }, dayLockByCut: { X: 0 } });
    var ord = orderOf(ops);
    var nb = neighbours(ord, 'X');
    assert(nb[0] === 'B' || nb[1] === 'B',
        '#4506: с галкой «Зафиксировать» место в дне выбирают веса — X встал рядом с B (те же ножи)',
        '(' + ord.join(' → ') + ')');
    assert(ord.indexOf('B') < ord.indexOf('C'),
        '#4506: зафиксированный сосед B остался на своём месте в дне, а не уехал в конец',
        '(' + ord.join(' → ') + ')');
    assert(dayOf(ops, 'X') === 0 && dayOf(ops, 'B') === 0,
        '#4506: день удержан — и перенесённое, и 🔒 остались в выбранном дне',
        '(X день ' + dayOf(ops, 'X') + ', B день ' + dayOf(ops, 'B') + ')');
})();
