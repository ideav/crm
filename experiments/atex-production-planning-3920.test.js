...
// не выталкивает соседей. Реальные нахлёсты (#3885) по-прежнему устраняются встык.
//
...
(function () {
    ...
    // Каждая резка стоит по своему planStart (как на Ганте); только 10:47 сдвинут встык за 08:00-окно.
    assertEqual(windows(sched), [
        { id: '239630', ws: 480, we: 662 },   // 08:00 → 11:02
        { id: '238161', ws: 662, we: 761 },   // 10:47 нахлёстывал 239630 → встык 11:02, конец 12:41
        { id: '237845', ws: 785, we: 894 },   // 13:05 → 14:54 (без сдвига)
        { id: '238013', ws: 958, we: 1005 }   // 15:58 → 16:45 (застрявшая — на своём месте, НЕ первой)
    ], '#3920 застрявшая резка с ранней «Очередностью» не выталкивает день в овертайм');
...
// ── 3. Настоящий нахлёст (#3885) по-прежнему устраняется встык (регрессия не ломает #3885) ──
(function () {
    var sched = scheduleFromStored([
        cut('188600', 480, 30, 15, 475),   // 08:00, окно до 16:40
        cut('191769', 480, 30, 15, 16)     // тоже 08:00 → встык после первого
    ], BASE);
    assertEqual(windows(sched), [
        { id: '188600', ws: 480, we: 1000 },
        { id: '191769', ws: 1000, we: 1061 }
    ], '#3885 сохранён: две резки на 08:00 — вторая встык за первой');
})();
