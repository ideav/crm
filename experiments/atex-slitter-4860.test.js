...
// Главного значения (самого номера, t82374) в полях НЕТ: `_m_set` его не пишет
// (docs/kb/crud.md #4906, #4925) — номер ставится при создании через `_m_new`.
// Реквизиты ищутся по имени; фото — файловый реквизит, в автосейв-поля не
// попадает (пустое стёрло бы загруженный файл); конечная длина — за вычетом
// расхода и списания (#4860).
var JUMBO_82374 = {
    id: '82374',
    reqs: [
...
assertEqual(parsedShort.fields, { 't9': '' }, '#4860 есть только знакомый реквизит — пишем его (без гл. значения, #4925)');
...
assertEqual(input.fields, { 't82382': '6', 't82384': '7', 't82386': '8', 't791708': '9', 't82378': '3' },
    '#4860 автосейв везёт расход, списание, браки и резки (#5005) — счётчики цепочки, длины и номер ведёт создание/отметка/завершение (#4925)');
