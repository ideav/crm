...
// #4421: намотка и лидер — РАЗНЫМИ строками (слитый «тайминг прохода» читался как норма,
// хотя нормой не был). Сумма та же: 212.4 + 236 = 448.4.
assert(/Намотка: 1\.8 \* 118 = 212\.4 мин/.test(details),
    'cutTimingDetails: «Намотка: 1.8 * 118 = 212.4 мин» (#4421 — своя строка)');
assert(/Лидер: 2 \* 118 = 236 мин/.test(details),
    'cutTimingDetails: «Лидер: 2 * 118 = 236 мин» (#4421 — своя строка)');
assert(/Итого резка: 212\.4 \+ 236 = 448\.4 мин/.test(details),
    'cutTimingDetails: «Итого резка: 212.4 + 236 = 448.4 мин» (лидер включён в итог)');
assert(!/Намотка и лидер/.test(details),
    'cutTimingDetails: слитой строки «Намотка и лидер» больше нет (#4421)');
...
assert(lines.indexOf('Намотка: 1.8 * 118 = 212.4 мин') >= 0,
    'timeline: «Намотка: 1.8 * 118 = 212.4 мин» (#4421)');
assert(/^Лидер: 236 мин/.test(lines.filter(function(t) { return /^Лидер: /.test(t); })[0] || ''),
    'timeline: лидер своей строкой; без нормы в ctx — только сумма (#4421)');
assert(/Итого резка: 212\.4 \+ 236 = 448\.4 мин/.test(joined),
    'timeline: «Итого резка: 212.4 + 236 = 448.4 мин» (лидер включён в итог)');
assert(!/Намотка и лидер/.test(joined),
    'timeline: слитой строки «Намотка и лидер» больше нет (#4421)');
