...
(function () {
    ...
    // между двумя 🔒 ОДНОГО дня; здесь их разделяет граница дней, поэтому запрета нет — и место X
    // выбирают веса. X режет то же сырьё и те же ножи, что F0, значит стои́т с ним ВПЛОТНУЮ
    // (с какой стороны — дело весов: обе точки стоят одинаково). Прежде эта строка требовала
    // ровно «F0 → X → F1»: упаковщик брал 🔒 первой всегда, и порядок §8 сюда не доходил.
    var chrono = ids(rows);
    var iF0 = chrono.indexOf('F0'), iX = chrono.indexOf('X');
    assert(Math.abs(iF0 - iX) === 1,
        'через границу дней свободное задание встаёт ВПЛОТНУЮ к 🔒 — запрета вставки тут нет',
        '(' + chrono.join(' → ') + ')');
    assert(chrono.indexOf('X') < chrono.indexOf('F1'),
        'X не вклинивается за 🔒 следующего дня', '(' + chrono.join(' → ') + ')');
})();
