...
// Свободные Y и Z — одна конфигурация, но между ними монолит 🔒. Сгруппировать их значило бы
// перенести Z ПЕРЕД 🔒, а это запрещено (#4497: перед 🔒 автоматика ничего не ставит) — группировка
// это штраф §8.2, замок оператора жёстче. Монолит при этом обязан остаться целым.
(function () {
    var day = [cut('Y', 'MA', [30, 29], 0), cut('F1', 'MB', [150, 7], 60, true),
    ...
    assert(got.indexOf('Z') > i2,
        '#4497: свободное Z, стоявшее ПОСЛЕ монолита, перед 🔒 не переносится', '(' + got.join(' → ') + ')');
})();
...
(function () {
    ...
    // #4487: правило проверяем как ПРАВИЛО, а не как одну раскладку. Монолит запрещает вставку
    // между двумя 🔒 ОДНОГО дня; здесь их разделяет граница дней, поэтому ЭТОГО запрета нет.
    // #4542 (решение заказчика 31.07.2026): место X решают уже не только веса — автоматика не
    // ОБГОНЯЕТ 🔒. В хранимом плане X стои́т ПОСЛЕ F1 (день 1, 09:00 против 08:00), значит и в
    // раскладке он остаётся после неё. Прежде эта строка требовала «X вплотную к F0» (день 0).
    var chrono = ids(rows);
    assert(chrono.indexOf('X') > chrono.indexOf('F1'),
        'через границу дней задание, стоявшее ПОСЛЕ 🔒, её не обгоняет (#4542)',
        '(' + chrono.join(' → ') + ')');

    // А вот стоявшее ПЕРЕД 🔒 следующего дня своё место сохраняет — запрета вставки на стыке дней
    // по-прежнему нет, и X2 встаёт вплотную к F0 (то же сырьё и ножи).
    var cuts2 = [cut('F0', 'MB', [150, 7], 0, true), cut('F1', 'MC', [60, 15], 24 * 60, true),
                 cut('X2', 'MB', [150, 7], 60)];
    var chrono2 = ids(plan(cuts2));
    var iF0b = chrono2.indexOf('F0'), iX2 = chrono2.indexOf('X2');
    assert(Math.abs(iF0b - iX2) === 1 && iX2 < chrono2.indexOf('F1'),
        'на стыке дней стоявшее перед 🔒 задание встаёт вплотную к 🔒 предыдущего дня',
        '(' + chrono2.join(' → ') + ')');
})();
...
// ── 8. День с ОДНИМ 🔒: её место тоже неприкосновенно (#4497) ───────────────────────────
// A и B — одна конфигурация, 🔒 F стои́т между ними. Группировка потребовала бы перенести B перед
// F — запрещено (#4497). Каждый остаётся по свою сторону замка, порядок дня сохраняется.
(function () {
    var day = [cut('A', 'MA', [30, 29], 0), cut('F', 'MB', [150, 7], 60, true),
               cut('B', 'MA', [30, 29], 120)];
    var got = ids(plan(day));
    assert(got.indexOf('A') < got.indexOf('F') && got.indexOf('B') > got.indexOf('F'),
        '#4497: свободные не переезжают через 🔒 — A перед ней, B после', '(' + got.join(' → ') + ')');
})();
