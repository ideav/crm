...
// Идеал: разные наборы ножей = {100},{100,60} → 2; разные сырья = A,B → 2. count=4, min=2*30+2*15=90.
eq(pq.ideal.knifeConfigs, 2, 'идеал: 2 разных набора ножей');
eq(pq.ideal.materials, 2, 'идеал: 2 разных сырья');
eq(pq.ideal.count, 4, 'идеал: 4 переналадки');
eq(pq.ideal.minutes, 90, 'идеал: 90 мин');
eq(pq.qualityAll.excessCount, 2 - 4, 'избыток (всё): факт 2 − идеал 4 = -2 (лучше идеала — вошли настроенными)');
