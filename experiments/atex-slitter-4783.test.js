...
(function() {
    ...
    // Список растёт: полноэкранные рабочие места добавляются в него по одному
    // (§3.5, #4799 добавил упаковщика; #4798 добавил втулкорез). Поэтому проверяем
    // не точный состав, а то, ради чего проверка существует: список объявлен
    // в head — до отрисовки body — и пульт слиттера в нём есть.
    var fullscreen = head.match(/FULLSCREEN_ACTIONS\s*=\s*\[([^\]]*)\]/);
    assert(!!fullscreen && /'slitter'/.test(fullscreen[1]),
...
(function() {
    ...
    // #4933 п.4 (решение заказчика): правая часть ПРОКРУЧИВАЕТСЯ сама — в ландшафтном
    // режиме планшета содержимое выше экрана, и без своего скролла нижние поля ввода
    // были недостижимы (страничный скролл на широком экране выключен, #4914).
    assert(/overflow-y:\s*auto/.test(rule('.atex-sl-main')),
        '#4933 п.4: правая часть прокручивается сама (ландшафт планшета)');
