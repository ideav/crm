...
class CabinetController {
    constructor() {
        ...
        // AI chat state
        this.aiChatStorageKey = 'cabinet_ai_chat_settings';
    ...
    populateUserData() {
        ...
        // Populate AI chat database picker
    ...
    getAiCommandPrompts() {
        return {
            base: [
                'Ты ИИ-помощник личного кабинета Интеграм.',
    ...
    loadAiServiceSettings() {
        try {
            ...
            console.warn('[cabinet] AI chat settings ignored:', err);
        }
    }
    ...
    saveAiServiceSettings() {
        ...
        try {
            ...
            showToast('Настройки ИИ-сервиса сохранены', 'success');
        } catch (err) {
            console.error('[cabinet] AI chat settings save failed:', err);
            showToast('Не удалось сохранить настройки ИИ-сервиса', 'error');
        }
    }
    ...
    connectAiService() {
        ...
        if (profile.tokenMode === 'own' && !profile.token) {
            showToast('Введите API token', 'error');
            return;
        }
    ...
    populateAiDatabaseSelect() {
        ...
        select.innerHTML = dbNames.map(dbName => {
            const label = dbName === 'my' ? 'Личный кабинет (my)' : dbName;
            return '<option value="' + this.escapeHtml(dbName) + '">' + this.escapeHtml(label) + '</option>';
        }).join('');
    ...
    getAccessibleAiDbs() {
        const dbNames = new Set(['my']);

        if (typeof CookieUtil !== 'undefined' && CookieUtil.getAllIdb) {
            CookieUtil.getAllIdb().forEach(dbName => {
                if (dbName) dbNames.add(dbName);
            });
        }

        this.databases.forEach(db => {
            if (db && db.DB) dbNames.add(db.DB);
        });
    ...
    getSelectedAiDb() {
        const select = document.getElementById('ai-target-db');
        return select && select.value ? select.value : 'my';
    }
    ...
    buildAiRequestPayload(message, commandType) {
        ...
        return {
            ...
            context: {
                ...
                userId: typeof uid !== 'undefined' ? uid : this.me
            },
    ...
    copyAiCommandPayload(commandId) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(payload).then(() => {
                showToast('JSON команды скопирован', 'success');
            }).catch(err => {
                console.error('[cabinet] AI command copy failed:', err);
    ...
    runAiCommandStub(commandId) {
        ...
        this.renderAiCommandQueue();
        showToast('Команда подготовлена к выполнению. Серверный обработчик будет добавлен позже.', 'info');
    }
